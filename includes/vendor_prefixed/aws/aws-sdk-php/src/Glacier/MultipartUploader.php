...
namespace WPStack_Connect_Vendor\Aws\Glacier;

use WPStack_Connect_Vendor\Aws\CommandInterface;
use WPStack_Connect_Vendor\Aws\HashingStream;
use WPStack_Connect_Vendor\Aws\Multipart\AbstractUploader;
use WPStack_Connect_Vendor\Aws\Multipart\UploadState;
use WPStack_Connect_Vendor\Aws\PhpHash;
 ...
 */
class MultipartUploader extends \WPStack_Connect_Vendor\Aws\Multipart\AbstractUploader
{
     ...
     * @return UploadState
     */
    public static function getStateFromService(\WPStack_Connect_Vendor\Aws\Glacier\GlacierClient $client, $vaultName, $uploadId, $accountId = '-')
    {
        $state = new \WPStack_Connect_Vendor\Aws\Multipart\UploadState(['accountId' => $accountId, 'vaultName' => $vaultName, 'uploadId' => $uploadId]);
        ...
        $state->setStatus(\WPStack_Connect_Vendor\Aws\Multipart\UploadState::INITIATED);
        return $state;
    }
    /**
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\Glacier\GlacierClient $client, $source, array $config = [])
    {
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::tryFopen($this->source->getMetadata('uri'), 'r');
            $body = $this->limitPartStream(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor($body));
            ...
            $body = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor();
            \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::copyToStream($source, $body);
        }
    ...
    protected function handleResult(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Aws\ResultInterface $result)
    {
    ...
    {
        $treeHash = new \WPStack_Connect_Vendor\Aws\Glacier\TreeHash();
        $archiveSize = 0;
     ...
     * @return Stream
     */
    private function decorateWithHashes(\WPStack_Connect_Vendor\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Make sure that a tree hash is calculated.
        $stream = new \WPStack_Connect_Vendor\Aws\HashingStream($stream, new \WPStack_Connect_Vendor\Aws\Glacier\TreeHash(), function ($result) use(&$data) {
            $data['checksum'] = \bin2hex($result);
        });
        // Make sure that a linear SHA256 hash is calculated.
        $stream = new \WPStack_Connect_Vendor\Aws\HashingStream($stream, new \WPStack_Connect_Vendor\Aws\PhpHash('sha256'), function ($result) use(&$data) {
