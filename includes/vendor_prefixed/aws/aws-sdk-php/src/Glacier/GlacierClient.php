 ...
 */
class GlacierClient extends \WPStack_Connect_Vendor\Aws\AwsClient
{
    public function __construct(array $args)
    {
        ...
        $stack->appendBuild(\WPStack_Connect_Vendor\Aws\Middleware::contentType(['UploadArchive', 'UploadPart']), 'glacier.content_type');
        $stack->appendInit(\WPStack_Connect_Vendor\Aws\Middleware::sourceFile($this->getApi(), 'body', 'sourceFile'), 'glacier.source_file');
    }
    /**
     * {@inheritdoc}
     *
        ...
        return function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                ...
                if (($name === 'UploadArchive' || $name === 'UploadMultipartPart') && (!$command['checksum'] || !$command['ContentSHA256'])) {
                    ...
                    if (!$command['checksum']) {
                        $body = new \WPStack_Connect_Vendor\Aws\HashingStream($body, new \WPStack_Connect_Vendor\Aws\Glacier\TreeHash(), function ($result) use(&$request) {
                    ...
                    if (!$command['ContentSHA256']) {
                        $body = new \WPStack_Connect_Vendor\Aws\HashingStream($body, new \WPStack_Connect_Vendor\Aws\PhpHash('sha256'), function ($result) use($command) {
