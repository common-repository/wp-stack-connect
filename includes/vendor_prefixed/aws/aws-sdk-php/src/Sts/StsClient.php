...
namespace WPStack_Connect_Vendor\Aws\Sts;
 ...
 */
class StsClient extends \WPStack_Connect_Vendor\Aws\AwsClient
{
    /**
     * {@inheritdoc}
     *
    ...
    {
        if (!isset($args['sts_regional_endpoints']) || $args['sts_regional_endpoints'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
            $args['sts_regional_endpoints'] = \WPStack_Connect_Vendor\Aws\Sts\RegionalEndpoints\ConfigurationProvider::defaultProvider($args);
        }
     ...
     */
    public function createCredentials(\WPStack_Connect_Vendor\Aws\Result $result)
    {
        if (!$result->hasKey('Credentials')) {
            throw new \InvalidArgumentException('Result contains no credentials');
        }
        $c = $result['Credentials'];
        return new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($c['AccessKeyId'], $c['SecretAccessKey'], isset($c['SessionToken']) ? $c['SessionToken'] : null, isset($c['Expiration']) && $c['Expiration'] instanceof \DateTimeInterface ? (int) $c['Expiration']->format('U') : null);
    }
    /**
