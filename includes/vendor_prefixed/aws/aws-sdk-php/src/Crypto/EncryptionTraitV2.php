...
trait EncryptionTraitV2
{
    ...
    private static $encryptClasses = ['gcm' => \WPStack_Connect_Vendor\Aws\Crypto\AesGcmEncryptingStream::class];
    /**
     ...
     * @internal
     */
    public function encrypt(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Stream $plaintext, array $options, \WPStack_Connect_Vendor\Aws\Crypto\MaterialsProviderV2 $provider, \WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\WPStack_Connect_Vendor\Aws\Crypto\MaterialsProviderV2::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $keys['Ciphertext'];
        unset($keys);
