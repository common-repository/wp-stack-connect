...
class AesGcm
{
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\Crypto\Polyfill\Key $aesKey, $keySize = 256, $blockSize = 8192)
    {
        /* Preconditions: */
     ...
     * @throws InvalidArgumentException
     */
    public static function encrypt($plaintext, $nonce, \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\Key $key, $aad, &$tag, $keySize = 256, $blockSize = 8192)
    {
        self::needs(self::strlen($nonce) === 12, 'Nonce must be exactly 12 bytes', \InvalidArgumentException::class);
        $encryptor = new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\AesGcm($key, $keySize, $blockSize);
        ...
        for ($i = 0; $i < $ciphertextLength; $i += $encryptor->blockSize) {
            $cBlock = new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray(self::substr($ciphertext, $i, $encryptor->blockSize));
            $gmac->update($cBlock);
        }
     ...
     * @throws InvalidArgumentException
     */
    public static function decrypt($ciphertext, $nonce, \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\Key $key, $aad, &$tag, $keySize = 256, $blockSize = 8192)
    {
        /* Precondition: */
        self::needs(self::strlen($nonce) === 12, 'Nonce must be exactly 12 bytes', \InvalidArgumentException::class);
        $encryptor = new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\AesGcm($key, $keySize, $blockSize);
        ...
        $expected = new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray($tag);
    ...
    {
        $gmac = new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\Gmac($this->aesKey, $nonce . "\x00\x00\x00\x01", $this->keySize);
        $aadBlock = new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray($aad);
