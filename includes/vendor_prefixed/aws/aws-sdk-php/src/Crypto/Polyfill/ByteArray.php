...
class ByteArray extends \SplFixedArray
{
    ...
    public function __construct($size = 0)
    {
        ...
        if (\is_array($size)) {
            ...
            $size = \count($arr);
        } elseif ($size instanceof \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray) {
            $arr = $size->toArray();
            $size = $size->count();
        } elseif (!\is_int($size)) {
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray(\pack('N', $num));
    }
    /**
     * @param ByteArray $other
     * @return bool
     */
    public function equals(\WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray $other)
    {
     ...
     * @return ByteArray
     */
    public function exclusiveOr(\WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray $array)
    {
     ...
     * @return ByteArray
     */
    public static function select($select, \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray $left, \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray $right)
    {
     ...
     * @return self
     */
    public function set(\WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray $input, $offset = 0, $length = null)
    {
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\Crypto\Polyfill\ByteArray(\array_slice($this->toArray(), $start, $length));
    }
    /**
