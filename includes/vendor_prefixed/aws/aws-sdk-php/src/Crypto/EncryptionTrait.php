...
use WPStack_Connect_Vendor\GuzzleHttp\Psr7\AppendStream;
...
trait EncryptionTrait
{
     ...
     * @internal
     */
    public function encrypt(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Stream $plaintext, array $cipherOptions, \WPStack_Connect_Vendor\Aws\Crypto\MaterialsProvider $provider, \WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\WPStack_Connect_Vendor\Aws\Crypto\MaterialsProvider::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $provider->encryptCek($cek, $materialsDescription);
        unset($cek);
        $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::IV_HEADER] = \base64_encode($cipherOptions['Iv']);
        $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::KEY_WRAP_ALGORITHM_HEADER] = $provider->getWrapAlgorithmName();
        $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::CONTENT_CRYPTO_SCHEME_HEADER] = $aesName;
        $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_LENGTH_HEADER] = \strlen($plaintext);
        $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER] = \json_encode($materialsDescription);
        if (!empty($cipherOptions['Tag'])) {
            $envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::CRYPTO_TAG_LENGTH_HEADER] = \strlen($cipherOptions['Tag']) * 8;
        }
     ...
     * @internal
     */
    protected function getEncryptingStream(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Stream $plaintext, $cek, &$cipherOptions)
    {
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                $cipherOptions['TagLength'] = 16;
                $cipherTextStream = new \WPStack_Connect_Vendor\Aws\Crypto\AesGcmEncryptingStream($plaintext, $cek, $cipherOptions['Iv'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : '', $cipherOptions['TagLength'], $cipherOptions['KeySize']);
                ...
                $appendStream = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\AppendStream([$cipherTextStream->createStream()]);
                $cipherOptions['Tag'] = $cipherTextStream->getTag();
                $appendStream->addStream(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor($cipherOptions['Tag']));
            ...
            default:
                ...
                $cipherTextStream = new \WPStack_Connect_Vendor\Aws\Crypto\AesEncryptingStream($plaintext, $cek, $cipherMethod);
