 ...
 * @deprecated
 */
class KmsMaterialsProvider extends \WPStack_Connect_Vendor\Aws\Crypto\MaterialsProvider implements \WPStack_Connect_Vendor\Aws\Crypto\MaterialsProviderInterface
{
    ...
    public function fromDecryptionEnvelope(\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect the materials description.');
        }
        $materialsDescription = \json_decode($envelope[\WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], \true);
