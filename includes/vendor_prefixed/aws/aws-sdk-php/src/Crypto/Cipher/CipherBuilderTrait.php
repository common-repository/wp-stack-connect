...
namespace WPStack_Connect_Vendor\Aws\Crypto\Cipher;
...
trait CipherBuilderTrait
{
    ...
    protected function buildCipherMethod($cipherName, $iv, $keySize)
    {
        switch ($cipherName) {
            case 'cbc':
                return new \WPStack_Connect_Vendor\Aws\Crypto\Cipher\Cbc($iv, $keySize);
            default:
                return null;
        }
    }
    ...
    protected function getCipherFromAesName($aesName)
    {
        switch ($aesName) {
            ...
            case 'AES/CBC/PKCS5Padding':
                return 'cbc';
            default:
                throw new \WPStack_Connect_Vendor\Aws\Exception\CryptoException('Unrecognized or unsupported' . ' AESName for reverse lookup.');
        }
    }
}
