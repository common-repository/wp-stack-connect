...
class TraceMiddleware
{
     ...
     *   "scrub_auth" is set to true.
     */
    public function __construct(array $config = [], \WPStack_Connect_Vendor\Aws\Api\Service $service = null)
    {
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\WPStack_Connect_Vendor\Aws\CommandInterface $cmd)
    {
        return ['instance' => \spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $this->getRedactedArray($cmd)];
    }
    private function requestArray(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    {
        return $value instanceof \WPStack_Connect_Vendor\Aws\ResultInterface ? ['instance' => \spl_object_hash($value), 'data' => $value->toArray()] : $value;
    }
    ...
    private function streamStr(\WPStack_Connect_Vendor\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\WPStack_Connect_Vendor\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = \fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\WPStack_Connect_Vendor\Aws\CommandInterface $command)
    {
     ...
     * @return array
     */
    private function getRedactedArray(\WPStack_Connect_Vendor\Aws\CommandInterface $cmd)
    {
