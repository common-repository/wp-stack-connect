...
use WPStack_Connect_Vendor\Aws\Api\ApiProvider;
use WPStack_Connect_Vendor\Aws\Api\Service;
use WPStack_Connect_Vendor\Aws\Api\Validator;
use WPStack_Connect_Vendor\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use WPStack_Connect_Vendor\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use WPStack_Connect_Vendor\Aws\ClientSideMonitoring\Configuration;
use WPStack_Connect_Vendor\Aws\Configuration\ConfigurationResolver;
use WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider;
use WPStack_Connect_Vendor\Aws\Credentials\Credentials;
use WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface;
use WPStack_Connect_Vendor\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use WPStack_Connect_Vendor\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
...
use WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use WPStack_Connect_Vendor\Aws\EndpointDiscovery\ConfigurationInterface;
use WPStack_Connect_Vendor\Aws\EndpointDiscovery\ConfigurationProvider;
use WPStack_Connect_Vendor\Aws\EndpointV2\EndpointDefinitionProvider;
use WPStack_Connect_Vendor\Aws\Exception\AwsException;
use WPStack_Connect_Vendor\Aws\Exception\InvalidRegionException;
use WPStack_Connect_Vendor\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use WPStack_Connect_Vendor\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use WPStack_Connect_Vendor\Aws\Signature\SignatureProvider;
use WPStack_Connect_Vendor\Aws\Token\Token;
use WPStack_Connect_Vendor\Aws\Token\TokenInterface;
use WPStack_Connect_Vendor\Aws\Token\TokenProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        // A value of 0 for the config option disables retries
        if ($value) {
            $config = \WPStack_Connect_Vendor\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \WPStack_Connect_Vendor\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\WPStack_Connect_Vendor\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\WPStack_Connect_Vendor\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    public static function _apply_defaults($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        $config = \WPStack_Connect_Vendor\Aws\DefaultsMode\ConfigurationProvider::unwrap($value);
        if ($config->getMode() !== 'legacy') {
    ...
    {
        return \WPStack_Connect_Vendor\Aws\Configuration\ConfigurationResolver::resolve('disable_request_compression', \false, 'bool', $args);
    }
    ...
    {
        return \WPStack_Connect_Vendor\Aws\Configuration\ConfigurationResolver::resolve('request_min_compression_size_bytes', 10240, 'int', $args);
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof \WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface) {
            $args['credentials'] = \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (\is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::fromCredentials(new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === \false) {
            $args['credentials'] = \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::fromCredentials(new \WPStack_Connect_Vendor\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
            $args['credentials'] = \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Credentials must be an instance of ' . "'" . \WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface::class . ', an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    public static function _default_credential_provider(array $args)
    {
        return \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_token($value, array &$args)
    {
        ...
        if ($value instanceof \WPStack_Connect_Vendor\Aws\Token\Token) {
            $args['token'] = \WPStack_Connect_Vendor\Aws\Token\TokenProvider::fromToken($value);
        } elseif (\is_array($value) && isset($value['token'])) {
            $args['token'] = \WPStack_Connect_Vendor\Aws\Token\TokenProvider::fromToken(new \WPStack_Connect_Vendor\Aws\Token\Token($value['token'], isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
            $args['token'] = \WPStack_Connect_Vendor\Aws\Token\TokenProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Token must be an instance of ' . \WPStack_Connect_Vendor\Aws\Token\TokenInterface::class . ', an associative ' . 'array that contains "token" and an optional "expires" ' . 'key-value pairs, a token provider function, or false.');
        }
    }
    public static function _default_token_provider(array $args)
    {
        return \WPStack_Connect_Vendor\Aws\Token\TokenProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        if ($value === \false) {
            $value = new \WPStack_Connect_Vendor\Aws\ClientSideMonitoring\Configuration(\false, \WPStack_Connect_Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \WPStack_Connect_Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \WPStack_Connect_Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\WPStack_Connect_Vendor\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\WPStack_Connect_Vendor\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \WPStack_Connect_Vendor\Aws\Api\Service(\WPStack_Connect_Vendor\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \WPStack_Connect_Vendor\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \WPStack_Connect_Vendor\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointProviderV2) {
                $options = self::getEndpointProviderOptions($args);
                $value = \WPStack_Connect_Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
            }
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            ...
            $args['region'] = \WPStack_Connect_Vendor\Aws\strip_fips_pseudo_regions($args['region']);
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = \WPStack_Connect_Vendor\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \WPStack_Connect_Vendor\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_use_fips_endpoint($value, array &$args)
    {
        if ($value instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
            $value = \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        if (\is_callable($value)) {
            $value = $value();
        }
        ...
        if ($value instanceof \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface) {
            $args['config']['use_fips_endpoint'] = $value;
        } else {
            // The Configuration class itself will validate other inputs
            $args['config']['use_fips_endpoint'] = new \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration($value);
        }
    }
    public static function _default_use_fips_endpoint(array &$args)
    {
        return \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_use_dual_stack_endpoint($value, array &$args)
    {
        if ($value instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
            $value = \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        if (\is_callable($value)) {
            $value = $value();
        }
        ...
        if ($value instanceof \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface) {
            $args['config']['use_dual_stack_endpoint'] = $value;
        } else {
            // The Configuration class itself will validate other inputs
            $args['config']['use_dual_stack_endpoint'] = new \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration($value, $args['region']);
        }
    }
    public static function _default_use_dual_stack_endpoint(array &$args)
    {
        return \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_debug($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        if ($value !== \false) {
            $list->interpose(new \WPStack_Connect_Vendor\Aws\TraceMiddleware($value === \true ? [] : $value, $args['api']));
        }
    }
    public static function _apply_stats($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\WPStack_Connect_Vendor\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        if ($value === \false) {
            return;
        }
        $validator = $value === \true ? new \WPStack_Connect_Vendor\Aws\Api\Validator() : new \WPStack_Connect_Vendor\Aws\Api\Validator($value);
        $list->appendValidate(\WPStack_Connect_Vendor\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \WPStack_Connect_Vendor\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        $args['handler'] = new \WPStack_Connect_Vendor\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($inputUserAgent, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        //Add SDK version
        $userAgent = ['aws-sdk-php/' . \WPStack_Connect_Vendor\Aws\Sdk::VERSION];
        ...
        if (isset($args['endpoint_discovery'])) {
            if ($args['endpoint_discovery'] instanceof \WPStack_Connect_Vendor\Aws\EndpointDiscovery\Configuration && $args['endpoint_discovery']->isEnabled()) {
        ...
        if (isset($args['retries'])) {
            if ($args['retries'] instanceof \WPStack_Connect_Vendor\Aws\Retry\Configuration) {
        ...
        $list->appendBuild(static function (callable $handler) use($userAgent) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request) use($handler, $userAgent) {
    ...
    public static function _apply_endpoint($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        $args['endpoint'] = $value;
    }
    public static function _apply_idempotency_auto_fill($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        $enabled = \false;
        $generator = null;
        if (\is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\WPStack_Connect_Vendor\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            $ruleset = \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointDefinitionProvider::getEndpointRuleset($service->getServiceName(), $service->getApiVersion());
            return new \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointProviderV2($ruleset, \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
        return \WPStack_Connect_Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \WPStack_Connect_Vendor\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \WPStack_Connect_Vendor\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : \call_user_func(\WPStack_Connect_Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \WPStack_Connect_Vendor\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
...
EOT;
    ...
    private static function isValidService($service)
    {
        ...
        $services = \WPStack_Connect_Vendor\Aws\manifest();
