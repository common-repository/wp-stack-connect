...
class ResultPaginator implements \Iterator
{
     ...
     * @param array              $config
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, $operation, array $args, array $config)
    {
        $this->client = $client;
    ...
    {
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () use($handleResult) {
            $nextToken = null;
            do {
                ...
                if ($retVal !== null) {
                    (yield \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($retVal));
                }
            } while ($nextToken);
        });
    }
    ...
    public function search($expression)
    {
        ...
        return flatmap($this, function (\WPStack_Connect_Vendor\Aws\Result $result) use($expression) {
    ...
    private function determineNextToken(\WPStack_Connect_Vendor\Aws\Result $result)
    {
