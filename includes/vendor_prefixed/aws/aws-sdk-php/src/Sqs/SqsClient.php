...
namespace WPStack_Connect_Vendor\Aws\Sqs;

use WPStack_Connect_Vendor\Aws\AwsClient;
use WPStack_Connect_Vendor\Aws\CommandInterface;
use WPStack_Connect_Vendor\Aws\Sqs\Exception\SqsException;
 ...
 */
class SqsClient extends \WPStack_Connect_Vendor\Aws\AwsClient
{
        ...
        return static function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $c, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $r = null) use($handler) {
                if ($c->hasParam('QueueUrl')) {
                    $r = $r->withUri(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\UriResolver::resolve($r->getUri(), new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($c['QueueUrl'])));
                }
                return $handler($c, $r);
            };
        };
                ...
                return $handler($c, $r)->then(function ($result) use($c, $r) {
                    foreach ((array) $result['Messages'] as $msg) {
                        ...
                        if (isset($msg['MD5OfBody']) && $bodyMd5 !== $msg['MD5OfBody']) {
                            throw new \WPStack_Connect_Vendor\Aws\Sqs\Exception\SqsException(\sprintf('MD5 mismatch. Expected %s, found %s', $msg['MD5OfBody'], $bodyMd5), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                        }
                        if (isset($msg['MD5OfMessageAttributes'])) {
                            ...
                            if ($messageAttributesMd5 !== $msg['MD5OfMessageAttributes']) {
                                throw new \WPStack_Connect_Vendor\Aws\Sqs\Exception\SqsException(\sprintf('Attribute MD5 mismatch. Expected %s, found %s', $msg['MD5OfMessageAttributes'], $messageAttributesMd5 ? $messageAttributesMd5 : 'No Attributes'), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                            }
                        } else {
                            if (isset($msg['MessageAttributes'])) {
                                throw new \WPStack_Connect_Vendor\Aws\Sqs\Exception\SqsException(\sprintf('No Attribute MD5 found. Expected %s', self::calculateMessageAttributesMd5($msg)), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                            }
                        }
                    }
                    return $result;
