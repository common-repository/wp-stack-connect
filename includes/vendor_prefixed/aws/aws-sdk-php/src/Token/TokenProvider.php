...
class TokenProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['token']) && $config['token'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
        ...
        return self::memoize(\call_user_func_array([\WPStack_Connect_Vendor\Aws\Token\TokenProvider::class, 'chain'], \array_values($defaultChain)));
    }
    /**
     ...
     * @return callable
     */
    public static function fromToken(\WPStack_Connect_Vendor\Aws\Token\TokenInterface $token)
    {
        $promise = \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($token);
    ...
    public static function chain()
    {
        ...
        if (empty($links)) {
            return function () {
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(\false);
            };
        }
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\WPStack_Connect_Vendor\Aws\Token\TokenInterface $token) use($provider, &$isConstant, &$result) {
                ...
                $result = null;
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(null);
            });
        };
    }
    /**
    ...
    public static function cache(callable $provider, \WPStack_Connect_Vendor\Aws\CacheInterface $cache, $cacheKey = null)
    {
        ...
        return function () use($provider, $cache, $cacheKey) {
            ...
            if (\is_array($found) && isset($found['token'])) {
                if (isset($found['token']) && $found['token'] instanceof \WPStack_Connect_Vendor\Aws\Token\TokenInterface) {
                    $foundToken = $found['token'];
                    if (!$foundToken->isExpired()) {
                        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($foundToken);
                    }
                    if (isset($found['refreshMethod']) && \is_callable($found['refreshMethod'])) {
                        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($found['refreshMethod']());
                    }
                }
            }
            return $provider()->then(function (\WPStack_Connect_Vendor\Aws\Token\TokenInterface $token) use($cache, $cacheKey) {
    ...
    {
        return new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise(new \WPStack_Connect_Vendor\Aws\Exception\TokenException($msg));
    }
    /**
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\Token\SsoTokenProvider($profileName, $filename, $config);
    }
}
