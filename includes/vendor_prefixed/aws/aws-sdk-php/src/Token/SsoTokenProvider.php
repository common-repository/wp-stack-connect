...
use WPStack_Connect_Vendor\Aws\Exception\TokenException;
use WPStack_Connect_Vendor\GuzzleHttp\Promise;
/**
 * Token that comes from the SSO provider
 */
class SsoTokenProvider implements \WPStack_Connect_Vendor\Aws\Token\RefreshableTokenProviderInterface
{
    use ParsesIniTrait;
    ...
    {
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            if (!@\is_readable($this->filename)) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\TokenException("Cannot read profiles from {$this->filename}");
            }
            $profiles = self::loadProfiles($this->filename);
            if (!isset($profiles[$this->ssoProfileName])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\TokenException("Profile {$this->ssoProfileName} does not exist in {$this->filename}.");
            }
            $ssoProfile = $profiles[$this->ssoProfileName];
            if (empty($ssoProfile['sso_session'])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\TokenException("Profile {$this->ssoProfileName} in {$this->filename} must contain an sso_session.");
            }
    ...
    public function refresh()
    {
        try {
            ...
            $token = $this();
            if ($token instanceof \WPStack_Connect_Vendor\Aws\Token\SsoToken && !$token->shouldAttemptRefresh()) {
                return $token;
            }
        } finally {
            ...
            if (empty($this->ssoOidcClient) || empty($tokenData['startUrl'])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\TokenException("Cannot refresh this token without an 'ssooidcClient' " . "and a 'start_url'");
            }
            ...
            if ($response['@metadata']['statusCode'] == 200) {
                ...
                $token = new \WPStack_Connect_Vendor\Aws\Token\SsoToken($tokenData['accessToken'], $tokenData['expiresAt'], $tokenData['refreshToken'], isset($tokenData['clientId']) ? $tokenData['clientId'] : null, isset($tokenData['clientSecret']) ? $tokenData['clientSecret'] : null, isset($tokenData['registrationExpiresAt']) ? $tokenData['registrationExpiresAt'] : null, isset($tokenData['region']) ? $tokenData['region'] : null, isset($tokenData['startUrl']) ? $tokenData['startUrl'] : null);
    ...
    private function validateTokenData($tokenLocation, $tokenData)
    {
        if (empty($tokenData['accessToken']) || empty($tokenData['expiresAt'])) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\TokenException("Token file at {$tokenLocation} must contain an access token and an expiration");
        }
        $expiration = \strtotime($tokenData['expiresAt']);
        if ($expiration === \false) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\TokenException("Cached SSO token returned an invalid expiration");
        } elseif ($expiration < \time()) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\TokenException("Cached SSO token returned an expired token");
        }
        return $tokenData;
    }
    /**
