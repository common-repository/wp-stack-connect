...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \WPStack_Connect_Vendor\Aws\Handler\GuzzleV6\GuzzleHandler();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return new \WPStack_Connect_Vendor\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \WPStack_Connect_Vendor\GuzzleHttp\default_user_agent();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return \WPStack_Connect_Vendor\GuzzleHttp\Client::getDefaultUserAgent();
    }
    throw new \RuntimeException('Unknown Guzzle version: ' . $version);
}
/**
...
function guzzle_major_version()
{
    ...
    if (\defined('WPStack_Connect_Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
        $version = (string) \WPStack_Connect_Vendor\GuzzleHttp\ClientInterface::VERSION;
        if ($version[0] === '6') {
            return $cache = 6;
        }
        if ($version[0] === '5') {
    ...
    } elseif (\defined('WPStack_Connect_Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
        return $cache = \WPStack_Connect_Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
    }
 ...
 * @throws \RuntimeException
 */
function serialize(\WPStack_Connect_Vendor\Aws\CommandInterface $command)
{
    $request = null;
    $handlerList = $command->getHandlerList();
    // Return a mock result.
    $handlerList->setHandler(function (\WPStack_Connect_Vendor\Aws\CommandInterface $_, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $r) use(&$request) {
        $request = $r;
        return new \WPStack_Connect_Vendor\GuzzleHttp\Promise\FulfilledPromise(new \WPStack_Connect_Vendor\Aws\Result([]));
    });
    \call_user_func($handlerList->resolve(), $command)->wait();
    if (!$request instanceof \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface) {
