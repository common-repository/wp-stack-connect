...
use WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine;
 ...
 */
class Waiter implements \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface
{
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, $name, array $args = [], array $config = [])
    {
        $this->client = $client;
        $this->name = $name;
        $this->args = $args;
        ...
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            ...
            for ($state = 'retry', $attempt = 1; $state === 'retry'; $attempt++) {
                ...
                if ($state === 'success') {
                    ...
                    (yield new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException($msg)));
                } elseif ($state === 'retry' && $attempt >= $this->config['maxAttempts']) {
                    $state = 'failed';
                    (yield new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException("The {$this->name} waiter failed after attempt #{$attempt}.")));
                }
            }
        });
    }
    ...
    {
        return !$result instanceof \WPStack_Connect_Vendor\Aws\ResultInterface ? \false : $acceptor['expected'] == $result->search($acceptor['argument']);
    }
    /**
     * @param Result $result   Result or exception.
     * @param array  $acceptor Acceptor configuration being checked.
    ...
    {
        if (!$result instanceof \WPStack_Connect_Vendor\Aws\ResultInterface) {
            return \false;
        }
    ...
    private function matchesStatus($result, array $acceptor)
    {
        ...
        if ($result instanceof \WPStack_Connect_Vendor\Aws\Exception\AwsException && ($response = $result->getResponse())) {
