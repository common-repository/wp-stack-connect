...
use WPStack_Connect_Vendor\Aws\Token\TokenAuthorization;
...
use WPStack_Connect_Vendor\GuzzleHttp\Psr7\LazyOpenStream;
...
final class Middleware
{
     ...
     * @return callable
     */
    public static function sourceFile(\WPStack_Connect_Vendor\Aws\Api\Service $api, $bodyParameter = 'Body', $sourceParameter = 'SourceFile')
    {
        return function (callable $handler) use($api, $bodyParameter, $sourceParameter) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler, $api, $bodyParameter, $sourceParameter) {
                $operation = $api->getOperation($command->getName());
                $source = $command[$sourceParameter];
                if ($source !== null && $operation->getInput()->hasMember($bodyParameter)) {
                    $command[$bodyParameter] = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\LazyOpenStream($source, 'r');
     ...
     * @return callable
     */
    public static function validation(\WPStack_Connect_Vendor\Aws\Api\Service $api, \WPStack_Connect_Vendor\Aws\Api\Validator $validator = null)
    {
        $validator = $validator ?: new \WPStack_Connect_Vendor\Aws\Api\Validator();
        return function (callable $handler) use($api, $validator) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($api, $validator, $handler) {
                if ($api->isModifiedModel()) {
                    $api = new \WPStack_Connect_Vendor\Aws\Api\Service($api->getDefinition(), $api->getProvider());
                }
                $operation = $api->getOperation($command->getName());
    ...
    public static function requestBuilder($serializer, $endpointProvider = null, array $providerArgs = null)
    {
        return function (callable $handler) use($serializer, $endpointProvider, $providerArgs) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command) use($serializer, $handler, $endpointProvider, $providerArgs) {
    ...
    public static function signer(callable $credProvider, callable $signatureFunction, $tokenProvider = null)
    {
        return function (callable $handler) use($signatureFunction, $credProvider, $tokenProvider) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request) use($handler, $signatureFunction, $credProvider, $tokenProvider) {
                $signer = $signatureFunction($command);
                if ($signer instanceof \WPStack_Connect_Vendor\Aws\Token\TokenAuthorization) {
                    return $tokenProvider()->then(function (\WPStack_Connect_Vendor\Aws\Token\TokenInterface $token) use($handler, $command, $signer, $request) {
                        return $handler($command, $signer->authorizeRequest($request, $token));
                    });
                } else {
                    return $credProvider()->then(function (\WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $creds) use($handler, $command, $signer, $request) {
    ...
    public static function tap(callable $fn)
    {
        return function (callable $handler) use($fn) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler, $fn) {
                $fn($command, $request);
                return $handler($command, $request);
            };
        };
    ...
    {
        $decider = $decider ?: \WPStack_Connect_Vendor\Aws\RetryMiddleware::createDefaultDecider();
        $delay = $delay ?: [\WPStack_Connect_Vendor\Aws\RetryMiddleware::class, 'exponentialDelay'];
        return function (callable $handler) use($decider, $delay, $stats) {
            return new \WPStack_Connect_Vendor\Aws\RetryMiddleware($decider, $delay, $handler, $stats);
        };
    }
    /**
    ...
    public static function invocationId()
    {
        return function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request) use($handler) {
    ...
    public static function contentType(array $operations)
    {
        return function (callable $handler) use($operations) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler, $operations) {
                if (!$request->hasHeader('Content-Type') && \in_array($command->getName(), $operations, \true) && ($uri = $request->getBody()->getMetadata('uri'))) {
                    $request = $request->withHeader('Content-Type', \WPStack_Connect_Vendor\GuzzleHttp\Psr7\MimeType::fromFilename($uri) ?: 'application/octet-stream');
                }
                return $handler($command, $request);
            };
        };
     ...
     * @return callable
     */
    public static function history(\WPStack_Connect_Vendor\Aws\History $history)
    {
        return function (callable $handler) use($history) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler, $history) {
                ...
                return $handler($command, $request)->then(function ($result) use($history, $ticket) {
                    ...
                    return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::rejectionFor($reason);
                });
            };
        };
    }
    ...
    public static function mapRequest(callable $f)
    {
        return function (callable $handler) use($f) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler, $f) {
    ...
    public static function timer()
    {
        return function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                $start = \microtime(\true);
                return $handler($command, $request)->then(function (\WPStack_Connect_Vendor\Aws\ResultInterface $res) use($start) {
                    ...
                    if ($err instanceof \WPStack_Connect_Vendor\Aws\Exception\AwsException) {
                        $err->setTransferInfo(['total_time' => \microtime(\true) - $start] + $err->getTransferInfo());
                    }
                    return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::rejectionFor($err);
                });
            };
        };
    }
