...
namespace WPStack_Connect_Vendor\Aws\MachineLearning;
 ...
 */
class MachineLearningClient extends \WPStack_Connect_Vendor\Aws\AwsClient
{
        ...
        return static function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                if ($command->getName() === 'Predict') {
                    $request = $request->withUri(new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($command['PredictEndpoint']));
                }
