...
use WPStack_Connect_Vendor\Aws\EndpointDiscovery\Exception\ConfigurationException;
...
class ConfigurationProvider extends \WPStack_Connect_Vendor\Aws\AbstractConfigurationProvider implements \WPStack_Connect_Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStack_Connect_Vendor\Aws\EndpointDiscovery\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStack_Connect_Vendor\Aws\EndpointDiscovery\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        $memo = self::memoize(\call_user_func_array([\WPStack_Connect_Vendor\Aws\EndpointDiscovery\ConfigurationProvider::class, 'chain'], $configProviders));
        if (isset($config['endpoint_discovery']) && $config['endpoint_discovery'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
    ...
    public static function env($cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        return function () use($cacheLimit) {
            ...
            if ($enabled !== \false && $enabled !== '') {
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\EndpointDiscovery\Configuration($enabled, $cacheLimit));
            }
    ...
    public static function fallback($config = [])
    {
        ...
        return function () use($enabled) {
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\EndpointDiscovery\Configuration($enabled, self::DEFAULT_CACHE_LIMIT));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null, $cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        ...
        return function () use($profile, $filename, $cacheLimit) {
            ...
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\EndpointDiscovery\Configuration($data[$profile]['endpoint_discovery_enabled'], $cacheLimit));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \WPStack_Connect_Vendor\Aws\EndpointDiscovery\ConfigurationInterface) {
            return $config;
        } elseif (\is_array($config) && isset($config['enabled'])) {
            if (isset($config['cache_limit'])) {
                return new \WPStack_Connect_Vendor\Aws\EndpointDiscovery\Configuration($config['enabled'], $config['cache_limit']);
            }
            return new \WPStack_Connect_Vendor\Aws\EndpointDiscovery\Configuration($config['enabled'], self::DEFAULT_CACHE_LIMIT);
        }
