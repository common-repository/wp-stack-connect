...
namespace WPStack_Connect_Vendor\Aws\EndpointDiscovery;
...
class EndpointDiscoveryMiddleware
{
    ...
    public function __construct(callable $handler, \WPStack_Connect_Vendor\Aws\AwsClient $client, array $args, $config)
    {
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $cmd, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (isset($op['endpointdiscovery'])) {
            $config = \WPStack_Connect_Vendor\Aws\EndpointDiscovery\ConfigurationProvider::unwrap($this->config);
            $isRequired = !empty($op['endpointdiscovery']['required']);
            if ($isRequired && !$config->isEnabled()) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('This operation ' . 'requires the use of endpoint discovery, but this has ' . 'been disabled in the configuration. Enable endpoint ' . 'discovery or use a different operation.');
            }
            // Continue only if enabled by config
            if ($config->isEnabled()) {
                if (isset($op['endpointoperation'])) {
                    throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('This operation is ' . 'contradictorily marked both as using endpoint discovery ' . 'and being the endpoint discovery operation. Please ' . 'verify the accuracy of your model files.');
                }
                ...
                // Check/create cache
                if (!isset(self::$cache)) {
                    self::$cache = new \WPStack_Connect_Vendor\Aws\LruArrayCache($config->getCacheLimit());
                }
                if (empty($endpointList = self::$cache->get($cacheKey))) {
                    $endpointList = new \WPStack_Connect_Vendor\Aws\EndpointDiscovery\EndpointList([]);
                }
                ...
                $g = function ($value) use($cacheKey, $cmd, $identifiers, $isRequired, $originalUri, $request, &$endpoint, &$g) {
                    if ($value instanceof \WPStack_Connect_Vendor\Aws\Exception\AwsException && ($value->getAwsErrorCode() == 'InvalidEndpointException' || $value->getStatusCode() == 421)) {
    ...
    private function discoverEndpoint($cacheKey, \WPStack_Connect_Vendor\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (isset($result['Endpoints'])) {
            ...
            $endpointList = new \WPStack_Connect_Vendor\Aws\EndpointDiscovery\EndpointList($endpointData);
            self::$cache->set($cacheKey, $endpointList);
            return $endpointList->getEndpoint();
        }
        throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('The endpoint discovery operation ' . 'yielded a response that did not contain properly formatted ' . 'endpoint data.');
    }
    private function getCacheKey(\WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $creds, \WPStack_Connect_Vendor\Aws\CommandInterface $cmd, array $identifiers)
    {
    ...
    private function getDiscoveryCommand(\WPStack_Connect_Vendor\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (!isset($endpointOperation)) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('This command is set to use ' . 'endpoint discovery, but no endpoint discovery operation was ' . 'found. Please verify the accuracy of your model files.');
        }
        $params = [];
        if (!empty($identifiers)) {
        ...
        $command->getHandlerList()->appendBuild(\WPStack_Connect_Vendor\Aws\Middleware::mapRequest(function (\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $r) {
    ...
    private function handleDiscoveryException($isRequired, $originalUri, \Exception $e, \WPStack_Connect_Vendor\Aws\CommandInterface $cmd, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($isRequired) {
            ...
            throw new \WPStack_Connect_Vendor\Aws\Exception\AwsException($message, $cmd, ['code' => 'EndpointDiscoveryException', 'message' => $message], $e);
        }
    ...
    private function handleInvalidEndpoint($cacheKey, $cmd, $identifiers, $isRequired, $originalUri, $request, $value, &$endpoint, &$g)
    {
        ...
        if ($endpointList instanceof \WPStack_Connect_Vendor\Aws\EndpointDiscovery\EndpointList) {
    ...
    private function modifyRequest(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, $endpoint)
    {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException("The supplied endpoint '" . "{$endpoint}' is invalid.");
    }
    private function useOriginalUri(\WPStack_Connect_Vendor\Psr\Http\Message\UriInterface $uri, \WPStack_Connect_Vendor\Aws\CommandInterface $cmd, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
