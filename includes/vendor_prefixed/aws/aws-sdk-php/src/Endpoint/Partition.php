...
namespace WPStack_Connect_Vendor\Aws\Endpoint;

use ArrayAccess;
use WPStack_Connect_Vendor\Aws\HasDataTrait;
use WPStack_Connect_Vendor\Aws\Sts\RegionalEndpoints\ConfigurationProvider;
use WPStack_Connect_Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider as S3ConfigurationProvider;
use InvalidArgumentException as Iae;
/**
 * Default implementation of an AWS partition.
 */
final class Partition implements \ArrayAccess, \WPStack_Connect_Vendor\Aws\Endpoint\PartitionInterface
{
    use HasDataTrait;
    ...
    {
        return $service === 'sts' && \in_array($region, $this->stsLegacyGlobalRegions) && (empty($options['sts_regional_endpoints']) || \WPStack_Connect_Vendor\Aws\Sts\RegionalEndpoints\ConfigurationProvider::unwrap($options['sts_regional_endpoints'])->getEndpointsType() !== 'regional');
    }
    /**
    ...
    {
        return $service === 's3' && $region === 'us-east-1' && (empty($options['s3_us_east_1_regional_endpoint']) || \WPStack_Connect_Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider::unwrap($options['s3_us_east_1_regional_endpoint'])->getEndpointsType() !== 'regional');
    }
