...
class ConfigurationProvider extends \WPStack_Connect_Vendor\Aws\AbstractConfigurationProvider implements \WPStack_Connect_Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        $memo = self::memoize(\call_user_func_array([\WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider::class, 'chain'], $configProviders));
        if (isset($config['use_fips_endpoint']) && $config['use_fips_endpoint'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($useFipsEndpoint)) {
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration($useFipsEndpoint));
            }
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPStack_Connect_Vendor\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
            if ($data === \false) {
            ...
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration($data[$profile][self::INI_USE_FIPS_ENDPOINT]));
        };
    }
    /**
    ...
    public static function fallback($region)
    {
        return function () use($region) {
            ...
            if ($isFipsPseudoRegion) {
                $configuration = new \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration(\true);
            } else {
                $configuration = new \WPStack_Connect_Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration(\false);
            }
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($configuration);
        };
    }
}
