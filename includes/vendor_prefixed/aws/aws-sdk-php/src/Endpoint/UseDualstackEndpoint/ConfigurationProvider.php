...
class ConfigurationProvider extends \WPStack_Connect_Vendor\Aws\AbstractConfigurationProvider implements \WPStack_Connect_Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        $memo = self::memoize(\call_user_func_array([\WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider::class, 'chain'], $configProviders));
        if (isset($config['use_dual_stack_endpoint']) && $config['use_dual_stack_endpoint'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
    ...
    public static function env($region)
    {
        return function () use($region) {
            ...
            if (!empty($useDualstackEndpoint)) {
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration($useDualstackEndpoint, $region));
            }
    ...
    public static function ini($region, $profile = null, $filename = null)
    {
        ...
        return function () use($region, $profile, $filename) {
            ...
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration($data[$profile][self::INI_USE_DUAL_STACK_ENDPOINT], $region));
        };
    }
    /**
    ...
    {
        return function () use($region) {
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration(\false, $region));
        };
    }
}
