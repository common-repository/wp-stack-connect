...
use WPStack_Connect_Vendor\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
 ...
 */
class AwsClient implements \WPStack_Connect_Vendor\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $aliases;
    ...
    {
        return \WPStack_Connect_Vendor\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \WPStack_Connect_Vendor\Aws\HandlerList();
        $resolver = new \WPStack_Connect_Vendor\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \WPStack_Connect_Vendor\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', \WPStack_Connect_Vendor\Aws\Exception\AwsException::class];
        }
        $service = \substr($klass, \strrpos($klass, '\\') + 1, -6);
        return [\strtolower($service), "WPStack_Connect_Vendor\\Aws\\{$service}\\Exception\\{$service}Exception"];
    }
    private function addEndpointParameterMiddleware($args)
    {
        if (empty($args['disable_host_prefix_injection'])) {
            $list = $this->getHandlerList();
            $list->appendBuild(\WPStack_Connect_Vendor\Aws\EndpointParameterMiddleware::wrap($this->api), 'endpoint_parameter');
        }
    }
    private function addEndpointDiscoveryMiddleware($config, $args)
    {
        ...
        if (!isset($args['endpoint'])) {
            $list->appendBuild(\WPStack_Connect_Vendor\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware::wrap($this, $args, $config['endpoint_discovery']), 'EndpointDiscoveryMiddleware');
        }
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\WPStack_Connect_Vendor\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \WPStack_Connect_Vendor\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\WPStack_Connect_Vendor\Aws\Middleware::signer($this->credentialProvider, $resolver, $this->tokenProvider), 'signer');
    }
    private function addRequestCompressionMiddleware($config)
    {
        if (empty($config['disable_request_compression'])) {
            $list = $this->getHandlerList();
            $list->appendBuild(\WPStack_Connect_Vendor\Aws\RequestCompressionMiddleware::wrap($config), 'request-compression');
        }
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\WPStack_Connect_Vendor\Aws\Middleware::invocationId(), 'invocation-id');
    }
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \WPStack_Connect_Vendor\Aws\load_compiled_json($file);
    ...
    {
        $streamRequestPayloadMiddleware = \WPStack_Connect_Vendor\Aws\StreamRequestPayloadMiddleware::wrap($this->api);
    ...
    private function addRecursionDetection()
    {
        ...
        $this->handlerList->appendBuild(\WPStack_Connect_Vendor\Aws\Middleware::recursionDetection(), 'recursion-detection');
    }
    /**
    ...
    private function addRequestBuilder()
    {
        ...
        $handlerList->prependBuild(\WPStack_Connect_Vendor\Aws\Middleware::requestBuilder($serializer, $endpointProvider, $endpointArgs), 'builderV2');
    }
    /**
    ...
    {
        return $this->endpointProvider instanceof \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointProviderV2;
    }
    ...
    {
        $aliases = \WPStack_Connect_Vendor\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');
