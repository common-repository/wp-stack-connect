...
use WPStack_Connect_Vendor\Aws\CommandPool;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => \WPStack_Connect_Vendor\Aws\Exception\MultipartUploadException::class];
     ...
     * @param array  $config
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\WPStack_Connect_Vendor\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \WPStack_Connect_Vendor\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\WPStack_Connect_Vendor\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        if ($this->config['state'] instanceof \WPStack_Connect_Vendor\Aws\Multipart\UploadState) {
        ...
        $state = new \WPStack_Connect_Vendor\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\WPStack_Connect_Vendor\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\WPStack_Connect_Vendor\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \WPStack_Connect_Vendor\Aws\Result();
                });
            };
        };
    }
