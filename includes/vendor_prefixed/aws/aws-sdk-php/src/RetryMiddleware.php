...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3, $extraConfig = [])
    {
        ...
        return function ($retries, \WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, \WPStack_Connect_Vendor\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors, $extraConfig) {
            ...
            if ($retries >= $maxRetries) {
                if (!empty($error) && $error instanceof \WPStack_Connect_Vendor\Aws\Exception\AwsException && $isRetryable) {
    ...
    private static function isRetryable($result, $error, $retryCurlErrors, $extraConfig = [])
    {
        ...
        if (!$error instanceof \WPStack_Connect_Vendor\Aws\Exception\AwsException) {
            return \false;
        }
        ...
        if (\count($retryCurlErrors) && ($previous = $error->getPrevious()) && $previous instanceof \WPStack_Connect_Vendor\GuzzleHttp\Exception\RequestException) {
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null)
    {
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$monitoringEvents, &$g) {
            ...
            } elseif ($value instanceof \WPStack_Connect_Vendor\Aws\ResultInterface && !$decider($retries, $command, $request, $value, null)) {
