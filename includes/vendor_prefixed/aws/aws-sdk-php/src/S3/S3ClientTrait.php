...
use WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise;
...
trait S3ClientTrait
{
    ...
    {
        return (new \WPStack_Connect_Vendor\Aws\S3\ObjectUploader($this, $bucket, $key, $body, $acl, $options))->promise();
    }
    /**
    ...
    public function copyAsync($fromB, $fromK, $destB, $destK, $acl = 'private', array $opts = [])
    {
        ...
        return (new \WPStack_Connect_Vendor\Aws\S3\ObjectCopier($this, $source, $destination, $acl, $opts))->promise();
    }
    /**
    ...
    {
        \WPStack_Connect_Vendor\Aws\S3\StreamWrapper::register($this);
    }
    /**
    ...
    {
        \WPStack_Connect_Vendor\Aws\S3\StreamWrapper::register($this, 's3', null, \true);
    }
    /**
    ...
    {
        if (!$prefix && !$regex) {
            return new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException('A prefix or regex is required.'));
        }
        $params = ['Bucket' => $bucket, 'Prefix' => $prefix];
        $iter = $this->getIterator('ListObjects', $params);
        if ($regex) {
            $iter = \WPStack_Connect_Vendor\Aws\filter($iter, function ($c) use($regex) {
                return \preg_match($regex, $c['Key']);
            });
        }
        return \WPStack_Connect_Vendor\Aws\S3\BatchDelete::fromIterator($this, $bucket, $iter, $options)->promise();
    }
    /**
    ...
    public function uploadDirectoryAsync($directory, $bucket, $keyPrefix = null, array $options = [])
    {
        ...
        return (new \WPStack_Connect_Vendor\Aws\S3\Transfer($this, $directory, $d, $options))->promise();
    }
    /**
    ...
    public function downloadBucketAsync($directory, $bucket, $keyPrefix = '', array $options = [])
    {
        ...
        return (new \WPStack_Connect_Vendor\Aws\S3\Transfer($this, $s, $directory, $options))->promise();
    }
    /**
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        return $handler($command)->then(static function (\WPStack_Connect_Vendor\Aws\ResultInterface $result) {
            return $result['@metadata']['headers']['x-amz-bucket-region'];
        }, function (\WPStack_Connect_Vendor\Aws\Exception\AwsException $e) {
    ...
    private function determineBucketRegionFromExceptionBody(\WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response)
    {
        try {
    ...
    public function doesBucketExistV2($bucket, $accept403 = \false)
    {
        ...
        try {
            ...
            return \true;
        } catch (\WPStack_Connect_Vendor\Aws\S3\Exception\S3Exception $e) {
            if ($accept403 && $e->getStatusCode() === 403 || $e instanceof \WPStack_Connect_Vendor\Aws\S3\Exception\PermanentRedirectException) {
                return \true;
            }
     ...
     */
    private function checkExistenceWithCommand(\WPStack_Connect_Vendor\Aws\CommandInterface $command)
    {
        try {
            $this->execute($command);
            return \true;
     ...
     */
    public abstract function execute(\WPStack_Connect_Vendor\Aws\CommandInterface $command);
    /**
