...
class ConfigurationProvider extends \WPStack_Connect_Vendor\Aws\AbstractConfigurationProvider implements \WPStack_Connect_Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStack_Connect_Vendor\Aws\S3\UseArnRegion\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStack_Connect_Vendor\Aws\S3\UseArnRegion\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        $memo = self::memoize(\call_user_func_array([\WPStack_Connect_Vendor\Aws\S3\UseArnRegion\ConfigurationProvider::class, 'chain'], $configProviders));
        if (isset($config['use_arn_region']) && $config['use_arn_region'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($useArnRegion)) {
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\S3\UseArnRegion\Configuration($useArnRegion));
            }
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\S3\UseArnRegion\Configuration($data[$profile][self::INI_USE_ARN_REGION]));
        };
    }
    /**
    ...
    {
        return function () {
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\S3\UseArnRegion\Configuration(self::DEFAULT_USE_ARN_REGION));
        };
    }
}
