...
use WPStack_Connect_Vendor\Aws\S3\S3ClientInterface;
 ...
 * @deprecated
 */
class S3EncryptionMultipartUploader extends \WPStack_Connect_Vendor\Aws\S3\MultipartUploader
{
    ...
    {
        return \in_array($cipherName, \WPStack_Connect_Vendor\Aws\Crypto\AbstractCryptoClient::$supportedCiphers);
    }
    private $provider;
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
