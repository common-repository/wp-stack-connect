 ...
 */
class S3EncryptionMultipartUploaderV2 extends \WPStack_Connect_Vendor\Aws\S3\MultipartUploader
{
    ...
    {
        return \in_array($cipherName, \WPStack_Connect_Vendor\Aws\Crypto\AbstractCryptoClientV2::$supportedCiphers);
    }
    private $provider;
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($this->encrypt($this->source, $this->config ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
