...
use WPStack_Connect_Vendor\Aws\Crypto\DecryptionTraitV2;
...
use WPStack_Connect_Vendor\Aws\Crypto\AbstractCryptoClientV2;
use WPStack_Connect_Vendor\Aws\Crypto\EncryptionTraitV2;
use WPStack_Connect_Vendor\Aws\Crypto\MetadataEnvelope;
use WPStack_Connect_Vendor\Aws\Crypto\MaterialsProvider;
use WPStack_Connect_Vendor\Aws\Crypto\Cipher\CipherBuilderTrait;
 ...
 * </code>
 */
class S3EncryptionClientV2 extends \WPStack_Connect_Vendor\Aws\Crypto\AbstractCryptoClientV2
{
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\S3\S3Client $client, $instructionFileSuffix = null)
    {
    ...
    public function putObjectAsync(array $args)
    {
        ...
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($this->encrypt(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor($args['Body']), $args, $provider, $envelope))->then(function ($encryptedBodyStream) use($args) {
            $hash = new \WPStack_Connect_Vendor\Aws\PhpHash('sha256');
            $hashingEncryptedBodyStream = new \WPStack_Connect_Vendor\Aws\HashingStream($encryptedBodyStream, $hash, self::getContentShaDecorator($args));
    ...
    public function getObjectAsync(array $args)
    {
        ...
        if (!isset($args['@SecurityProfile']) || !\in_array($args['@SecurityProfile'], self::$supportedSecurityProfiles)) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\CryptoException("@SecurityProfile is required and must be" . " set to 'V2' or 'V2_AND_LEGACY'");
        }
