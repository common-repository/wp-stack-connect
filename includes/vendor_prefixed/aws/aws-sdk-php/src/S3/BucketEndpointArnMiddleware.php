...
use WPStack_Connect_Vendor\Aws\Arn\S3\OutpostsAccessPointArn;
use WPStack_Connect_Vendor\Aws\Arn\S3\MultiRegionAccessPointArn;
...
use WPStack_Connect_Vendor\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $cmd, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $req)
    {
        ...
        if (!empty($op['input']['shape'])) {
            ...
            if (!empty($input = $service['shapes'][$op['input']['shape']])) {
                ...
                if (!empty($arnableKey) && \WPStack_Connect_Vendor\Aws\Arn\ArnParser::isArn($cmd[$arnableKey])) {
                    try {
                        // Throw for commands that do not support ARN inputs
                        if (\in_array($cmd->getName(), $this->nonArnableCommands)) {
                            throw new \WPStack_Connect_Vendor\Aws\S3\Exception\S3Exception('ARN values cannot be used in the bucket field for' . ' the ' . $cmd->getName() . ' operation.', $cmd);
                        }
                        if (!$this->isUseEndpointV2) {
                            $arn = \WPStack_Connect_Vendor\Aws\Arn\ArnParser::parse($cmd[$arnableKey]);
                        ...
                        if ($arn instanceof \WPStack_Connect_Vendor\Aws\Arn\S3\OutpostsArnInterface || $arn instanceof \WPStack_Connect_Vendor\Aws\Arn\ObjectLambdaAccessPointArn) {
                            $cmd['@context']['signing_service'] = $arn->getService();
                        }
                    } catch (\WPStack_Connect_Vendor\Aws\Arn\Exception\InvalidArnException $e) {
                        // Add context to ARN exception
                        throw new \WPStack_Connect_Vendor\Aws\S3\Exception\S3Exception('Bucket parameter parsed as ARN and failed with: ' . $e->getMessage(), $cmd, [], $e);
                    }
                }
            }
        }
    ...
    private function generateAccessPointHost(\WPStack_Connect_Vendor\Aws\Arn\AccessPointArn $arn, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $req)
    {
        if ($arn instanceof \WPStack_Connect_Vendor\Aws\Arn\S3\OutpostsAccessPointArn) {
            $accesspointName = $arn->getAccesspointName();
        } else {
            $accesspointName = $arn->getResourceId();
        }
        if ($arn instanceof \WPStack_Connect_Vendor\Aws\Arn\S3\MultiRegionAccessPointArn) {
        ...
        $region = \WPStack_Connect_Vendor\Aws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof \WPStack_Connect_Vendor\Aws\Arn\AccessPointArnInterface) {
            ...
            if ($arn instanceof \WPStack_Connect_Vendor\Aws\Arn\S3\OutpostsAccessPointArn && !empty($this->config['dual_stack'])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Dualstack is currently not supported with S3 Outposts access' . ' points. Please disable dualstack or do not supply an' . ' access point ARN.');
            }
            if ($arn instanceof \WPStack_Connect_Vendor\Aws\Arn\S3\MultiRegionAccessPointArn) {
                if (!empty($this->config['disable_multiregion_access_points'])) {
                    throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Multi-Region Access Point ARNs are disabled, but one was provided.  Please' . ' enable them or provide a different ARN.');
                }
                if (!empty($this->config['dual_stack'])) {
                    throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Multi-Region Access Point ARNs do not currently support dual stack. Please' . ' disable dual stack or provide a different ARN.');
                }
            }
            // Accelerate is not supported with access points
            if (!empty($this->config['accelerate'])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Accelerate is currently not supported with access points.' . ' Please disable accelerate or do not supply an access' . ' point ARN.');
            }
            // Path-style is not supported with access points
            if (!empty($this->config['path_style'])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Path-style addressing is currently not supported with' . ' access points. Please disable path-style or do not' . ' supply an access point ARN.');
            }
            // Custom endpoint is not supported with access points
            if (!\is_null($this->config['endpoint']) && !$arn instanceof \WPStack_Connect_Vendor\Aws\Arn\ObjectLambdaAccessPointArn) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('A custom endpoint has been supplied along with an access' . ' point ARN, and these are not compatible with each other.' . ' Please only use one or the other.');
            }
            // Dualstack is not supported with object lambda access points
            if ($arn instanceof \WPStack_Connect_Vendor\Aws\Arn\ObjectLambdaAccessPointArn && !empty($this->config['dual_stack'])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Dualstack is currently not supported with Object Lambda access' . ' points. Please disable dualstack or do not supply an' . ' access point ARN.');
            }
            // Global endpoints do not support cross-region requests
            if ($this->isGlobal($this->region) && $this->config['use_arn_region']->isUseArnRegion() == \false && $arn->getRegion() != $this->region && !$arn instanceof \WPStack_Connect_Vendor\Aws\Arn\S3\MultiRegionAccessPointArn) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Global endpoints do not support cross region requests.' . ' Please enable use_arn_region or do not supply a global region' . ' with a different region in the ARN.');
            }
        ...
        throw new \WPStack_Connect_Vendor\Aws\Arn\Exception\InvalidArnException('Provided ARN was not a valid S3 access' . ' point ARN or S3 Outposts access point ARN.');
    }
    /**
