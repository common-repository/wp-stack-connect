...
use WPStack_Connect_Vendor\Aws\S3\Exception\S3MultipartUploadException;
/**
...
class MultipartUploader extends \WPStack_Connect_Vendor\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, \array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \WPStack_Connect_Vendor\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
    ...
    protected function extractETag(\WPStack_Connect_Vendor\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \WPStack_Connect_Vendor\GuzzleHttp\Psr7\MimeType::fromFilename($uri) ?: 'application/octet-stream';
        }
    }
    ...
    private function decorateWithHashes(\WPStack_Connect_Vendor\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        ...
        $hash = new \WPStack_Connect_Vendor\Aws\PhpHash('sha256');
        return new \WPStack_Connect_Vendor\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
