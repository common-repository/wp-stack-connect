...
use WPStack_Connect_Vendor\GuzzleHttp\Psr7\CachingStream;
...
class StreamWrapper
{
     ...
     */
    public static function register(\WPStack_Connect_Vendor\Aws\S3\S3ClientInterface $client, $protocol = 's3', \WPStack_Connect_Vendor\Aws\CacheInterface $cache = null, $v2Existence = \false)
    {
        ...
        if ($cache) {
            ...
            $default[$protocol]['cache'] = new \WPStack_Connect_Vendor\Aws\LruArrayCache();
        }
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\MimeType::fromFilename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \WPStack_Connect_Vendor\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (\WPStack_Connect_Vendor\Aws\Result $result) use($filterFn) {
    ...
    private function openReadStream()
    {
        ...
        if ($this->getOption('seekable') && !$this->body->isSeekable()) {
            $this->body = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\CachingStream($this->body);
        }
        return \true;
    }
    private function openWriteStream()
    {
        $this->body = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Stream(\fopen('php://temp', 'r+'));
        return \true;
    }
    ...
    {
        if (!$this->cache) {
            $this->cache = $this->getOption('cache') ?: new \WPStack_Connect_Vendor\Aws\LruArrayCache();
        }
        return $this->cache;
    }
    /**
