 ...
 */
class ValidateResponseChecksumParser extends \WPStack_Connect_Vendor\Aws\Api\Parser\AbstractParser
{
    use CalculatesChecksumTrait;
    /**
     * @param callable $parser Parser to wrap.
     */
    public function __construct(callable $parser, \WPStack_Connect_Vendor\Aws\Api\Service $api)
    {
        $this->api = $api;
        $this->parser = $parser;
    }
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        if ($checksumValidationInfo['status'] == "SUCCEEDED") {
            ...
            if ($checksumValidationInfo['status'] == "FAILED") {
                ...
                throw new \WPStack_Connect_Vendor\Aws\S3\Exception\S3Exception("Calculated response checksum did not match the expected value", $command);
            }
        }
        return $result;
    }
     ...
     */
    public function validateChecksum($checksumPriority, \WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response)
    {
     ...
     * @param ResponseInterface $response
     */
    public function chooseChecksumHeaderToValidate($checksumPriority, \WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response)
    {
