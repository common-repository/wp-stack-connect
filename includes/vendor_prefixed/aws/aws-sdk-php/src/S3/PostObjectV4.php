...
use WPStack_Connect_Vendor\Aws\Signature\SignatureTrait;
use WPStack_Connect_Vendor\Aws\Signature\SignatureV4 as SignatureV4;
use WPStack_Connect_Vendor\Aws\Api\TimestampShape as TimestampShape;
/**
...
class PostObjectV4
{
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\S3\S3ClientInterface $client, $bucket, array $formInputs, array $options = [], $expiration = '+1 hours')
    {
        $this->client = $client;
        $this->bucket = $bucket;
        ...
        // setup basic policy
        $policy = ['expiration' => \WPStack_Connect_Vendor\Aws\Api\TimestampShape::format($expiration, 'iso8601'), 'conditions' => $options];
    ...
    {
        $uri = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($this->client->getEndpoint());
    ...
    protected function getPolicyAndSignature(\WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $credentials, array $policy)
    {
        $ldt = \gmdate(\WPStack_Connect_Vendor\Aws\Signature\SignatureV4::ISO8601_BASIC);
        $sdt = \substr($ldt, 0, 8);
