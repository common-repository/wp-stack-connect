...
class ApplyChecksumMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\WPStack_Connect_Vendor\Aws\Api\Service $api)
    {
        return function (callable $handler) use($api) {
            return new self($handler, $api);
        };
    }
    public function __construct(callable $nextHandler, \WPStack_Connect_Vendor\Aws\Api\Service $api)
    {
        $this->api = $api;
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!empty($checksumInfo)) {
            ...
            if (!empty($checksumRequired) && !$request->hasHeader('Content-MD5') || \in_array($name, self::$sha256AndMd5) && $addContentMD5) {
                ...
                $request = $request->withHeader('Content-MD5', \base64_encode(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::hash($body, 'md5', \true)));
                return $next($command, $request);
            }
        }
