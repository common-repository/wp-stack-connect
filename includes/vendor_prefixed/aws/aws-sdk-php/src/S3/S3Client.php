...
use WPStack_Connect_Vendor\Aws\Api\DocModel;
...
use WPStack_Connect_Vendor\Aws\Command;
use WPStack_Connect_Vendor\Aws\Exception\AwsException;
use WPStack_Connect_Vendor\Aws\HandlerList;
use WPStack_Connect_Vendor\Aws\InputValidationMiddleware;
use WPStack_Connect_Vendor\Aws\Middleware;
use WPStack_Connect_Vendor\Aws\Retry\QuotaManager;
use WPStack_Connect_Vendor\Aws\RetryMiddleware;
use WPStack_Connect_Vendor\Aws\ResultInterface;
use WPStack_Connect_Vendor\Aws\CommandInterface;
use WPStack_Connect_Vendor\Aws\RetryMiddlewareV2;
...
use WPStack_Connect_Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use WPStack_Connect_Vendor\GuzzleHttp\Exception\RequestException;
use WPStack_Connect_Vendor\GuzzleHttp\Promise\Promise;
 ...
 */
class S3Client extends \WPStack_Connect_Vendor\Aws\AwsClient implements \WPStack_Connect_Vendor\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var array */
    ...
    {
        if (!isset($args['s3_us_east_1_regional_endpoint']) || $args['s3_us_east_1_regional_endpoint'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
            $args['s3_us_east_1_regional_endpoint'] = \WPStack_Connect_Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        $this->addBuiltIns($args);
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(\WPStack_Connect_Vendor\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\WPStack_Connect_Vendor\Aws\S3\ApplyChecksumMiddleware::wrap($this->getApi()), 's3.checksum');
        $stack->appendBuild(\WPStack_Connect_Vendor\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\WPStack_Connect_Vendor\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } elseif (!$this->isUseEndpointV2()) {
            $stack->appendBuild(\WPStack_Connect_Vendor\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), $this->getConfig('endpoint_provider'), ['accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'use_fips_endpoint' => $this->getConfig('use_fips_endpoint'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint')->isUseDualStackEndpoint()]), 's3.endpoint_middleware');
        }
        $stack->appendBuild(\WPStack_Connect_Vendor\Aws\S3\BucketEndpointArnMiddleware::wrap($this->getApi(), $this->getRegion(), ['use_arn_region' => $this->getConfig('use_arn_region'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint')->isUseDualStackEndpoint(), 'use_fips_endpoint' => $this->getConfig('use_fips_endpoint'), 'disable_multiregion_access_points' => $this->getConfig('disable_multiregion_access_points'), 'endpoint' => isset($args['endpoint']) ? $args['endpoint'] : null], $this->isUseEndpointV2()), 's3.bucket_endpoint_arn');
        $stack->appendValidate(\WPStack_Connect_Vendor\Aws\InputValidationMiddleware::wrap($this->getApi(), self::$mandatoryAttributes), 'input_validation_middleware');
        $stack->appendSign(\WPStack_Connect_Vendor\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\WPStack_Connect_Vendor\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\WPStack_Connect_Vendor\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public function createPresignedRequest(\WPStack_Connect_Vendor\Aws\CommandInterface $command, $expires, array $options = [])
    {
        $command = clone $command;
        $command->getHandlerList()->remove('signer');
        $request = \WPStack_Connect_Vendor\Aws\serialize($command);
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \WPStack_Connect_Vendor\Aws\serialize($command)->getUri();
    }
    /**
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\WPStack_Connect_Vendor\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\Command $command, $request = null) use($handler) {
    ...
    {
        return static function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\WPStack_Connect_Vendor\Aws\ResultInterface $result) use($autoSet) {
    ...
    {
        return static function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\Command $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request) use($handler) {
                $uri = $request->getUri();
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \WPStack_Connect_Vendor\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $maxRetries = $config->getMaxAttempts() - 1;
                $decider = \WPStack_Connect_Vendor\Aws\RetryMiddleware::createDefaultDecider($maxRetries);
                $decider = function ($retries, $command, $request, $result, $error) use($decider, $maxRetries) {
                    ...
                    if ($error instanceof \WPStack_Connect_Vendor\Aws\Exception\AwsException && $retries < $maxRetries) {
                        ...
                        if ($error->getPrevious() instanceof \WPStack_Connect_Vendor\GuzzleHttp\Exception\RequestException) {
                ...
                $delay = [\WPStack_Connect_Vendor\Aws\RetryMiddleware::class, 'exponentialDelay'];
                $list->appendSign(\WPStack_Connect_Vendor\Aws\Middleware::retry($decider, $delay), 'retry');
            } else {
                $defaultDecider = \WPStack_Connect_Vendor\Aws\RetryMiddlewareV2::createDefaultDecider(new \WPStack_Connect_Vendor\Aws\Retry\QuotaManager(), $config->getMaxAttempts());
                $list->appendSign(\WPStack_Connect_Vendor\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'decider' => function ($attempts, \WPStack_Connect_Vendor\Aws\CommandInterface $cmd, $result) use($defaultDecider, $config) {
                    $isRetryable = $defaultDecider($attempts, $cmd, $result);
                    if (!$isRetryable && $result instanceof \WPStack_Connect_Vendor\Aws\Exception\AwsException && $attempts < $config->getMaxAttempts()) {
                        ...
                        if ($result->getPrevious() instanceof \WPStack_Connect_Vendor\GuzzleHttp\Exception\RequestException && $cmd->getName() !== 'CompleteMultipartUpload') {
                            $isRetryable = \true;
                        }
                    }
                    return $isRetryable;
    ...
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        \WPStack_Connect_Vendor\Aws\ClientResolver::_apply_api_provider($value, $args);
        $args['parser'] = new \WPStack_Connect_Vendor\Aws\S3\GetBucketLocationParser(new \WPStack_Connect_Vendor\Aws\S3\ValidateResponseChecksumParser(new \WPStack_Connect_Vendor\Aws\S3\AmbiguousSuccessParser(new \WPStack_Connect_Vendor\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']), $args['api']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
    ...
    public static function applyDocFilters(array $api, array $docs)
    {
        ...
        return [new \WPStack_Connect_Vendor\Aws\Api\Service($api, \WPStack_Connect_Vendor\Aws\Api\ApiProvider::defaultProvider()), new \WPStack_Connect_Vendor\Aws\Api\DocModel($docs)];
    }
    /**
     * @internal
     * @codeCoverageIgnore
