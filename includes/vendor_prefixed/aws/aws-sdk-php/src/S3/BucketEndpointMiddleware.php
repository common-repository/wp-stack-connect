...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, \WPStack_Connect_Vendor\Aws\CommandInterface $command)
    {
        ...
        if ($key) {
            $path = \WPStack_Connect_Vendor\Aws\S3\S3Client::encodeKey(\rawurldecode($path));
        }
