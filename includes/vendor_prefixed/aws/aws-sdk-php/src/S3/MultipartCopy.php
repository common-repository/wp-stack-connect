...
use WPStack_Connect_Vendor\Aws\Multipart\AbstractUploadManager;
use WPStack_Connect_Vendor\Aws\ResultInterface;
use WPStack_Connect_Vendor\GuzzleHttp\Psr7;
class MultipartCopy extends \WPStack_Connect_Vendor\Aws\Multipart\AbstractUploadManager
{
    ...
    private function createPart($partNumber, $partsCount)
    {
        ...
        $uri = \WPStack_Connect_Vendor\Aws\Arn\ArnParser::isArn($bucket) ? '' : '/';
        $uri .= $bucket . '/' . $key;
        $data['CopySource'] = $uri;
    ...
    {
        if ($this->config['source_metadata'] instanceof \WPStack_Connect_Vendor\Aws\ResultInterface) {
        ...
        if (\is_array($this->source)) {
            ...
            if (\strpos($key, '?')) {
                ...
                $headParams['Key'] = $key;
                $query = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Query::parse($query, \false);
    ...
    {
        $sourceBuilder = \WPStack_Connect_Vendor\Aws\Arn\ArnParser::isArn($inputSource) ? '' : '/';
