...
use WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
/**
 ...
 */
class Transfer implements \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface
{
    private $client;
    private $promise;
    private $source;
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\S3\S3ClientInterface $client, $source, $dest, array $options = [])
    {
        $this->client = $client;
        ...
        $this->concurrency = isset($options['concurrency']) ? $options['concurrency'] : \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::DEFAULT_CONCURRENCY;
        $this->mupThreshold = isset($options['mup_threshold']) ? $options['mup_threshold'] : 16777216;
        if ($this->mupThreshold < \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MIN_SIZE) {
    ...
    private function createDownloadPromise()
    {
        ...
        foreach ($this->getDownloadsIterator() as $object) {
            ...
            if ($this->resolvesOutsideTargetDirectory($sink, $objectKey)) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\AwsException('Cannot download key ' . $objectKey . ', its relative path resolves outside the' . ' parent directory', $command);
            }
        ...
        return (new \WPStack_Connect_Vendor\Aws\CommandPool($this->client, $commands, ['concurrency' => $this->concurrency, 'before' => $this->before, 'rejected' => function ($reason, $idx, \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromiseInterface $p) {
            $p->reject($reason);
        }]))->promise();
    }
    private function createUploadPromise()
    {
        ...
        $files = \WPStack_Connect_Vendor\Aws\map($this->getUploadsIterator(), function ($file) {
        ...
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Each::ofLimitAll($files, $this->concurrency);
    }
    /** @return Iterator */
    private function getUploadsIterator()
    {
        if (\is_string($this->source)) {
            return \WPStack_Connect_Vendor\Aws\filter(\WPStack_Connect_Vendor\Aws\recursive_dir_iterator($this->sourceMetadata['path']), function ($file) {
                return !\is_dir($file);
            });
        }
        return $this->source;
    ...
    private function getDownloadsIterator()
    {
        if (\is_string($this->source)) {
            ...
            $files = \WPStack_Connect_Vendor\Aws\map($files, function ($key) use($listArgs) {
                return "s3://{$listArgs['Bucket']}/{$key}";
            });
            return \WPStack_Connect_Vendor\Aws\filter($files, function ($key) {
    ...
    private function uploadMultipart($filename)
    {
        ...
        return (new \WPStack_Connect_Vendor\Aws\S3\MultipartUploader($this->client, $filename, ['bucket' => $args['Bucket'], 'key' => $args['Key'], 'before_initiate' => $this->before, 'before_upload' => $this->before, 'before_complete' => $this->before, 'concurrency' => $this->concurrency, 'add_content_md5' => $this->addContentMD5]))->promise();
    }
    ...
    private function addDebugToBefore($debug)
    {
        ...
        $s3Args = $this->s3Args;
        $this->before = static function (\WPStack_Connect_Vendor\Aws\CommandInterface $command) use($before, $debug, $sourcePath, $s3Args) {
