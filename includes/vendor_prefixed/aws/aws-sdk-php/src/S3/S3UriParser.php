...
class S3UriParser
{
    ...
    public function parse($uri)
    {
        ...
        if (!empty($components)) {
            if (\WPStack_Connect_Vendor\Aws\Arn\ArnParser::isArn($components['host'])) {
                $arn = new \WPStack_Connect_Vendor\Aws\Arn\S3\AccessPointArn($components['host']);
                return ['bucket' => $components['host'], 'key' => $components['path'], 'path_style' => \false, 'region' => $arn->getRegion()];
            }
        }
        $url = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::uriFor($uri);
    ...
    private function parseStreamWrapper(\WPStack_Connect_Vendor\Psr\Http\Message\UriInterface $url)
    {
    ...
    private function parseCustomEndpoint(\WPStack_Connect_Vendor\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
    ...
    private function parsePathStyle(\WPStack_Connect_Vendor\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
        if ($url->getPath() != '/') {
            $path = \ltrim($url->getPath(), '/');
    ...
    private function parseVirtualHosted(\WPStack_Connect_Vendor\Psr\Http\Message\UriInterface $url, array $matches)
    {
        $result = self::$defaultResult;
        $result['path_style'] = \false;
