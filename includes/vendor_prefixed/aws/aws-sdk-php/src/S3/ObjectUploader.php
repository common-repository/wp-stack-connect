 ...
 * appropriate.
 */
class ObjectUploader implements \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface
{
     ...
     *                                          the sub command(s).
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\S3\S3ClientInterface $client, $bucket, $key, $body, $acl = 'private', array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
        $this->key = $key;
        $this->body = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor($body);
        $this->acl = $acl;
    ...
    public function promise()
    {
        ...
        if ($this->requiresMultipart($this->body, $mup_threshold)) {
            ...
            return (new \WPStack_Connect_Vendor\Aws\S3\MultipartUploader($this->client, $this->body, ['bucket' => $this->bucket, 'key' => $this->key, 'acl' => $this->acl] + $this->options))->promise();
        }
     ...
     * @return bool
     */
    private function requiresMultipart(\WPStack_Connect_Vendor\Psr\Http\Message\StreamInterface &$body, $threshold)
    {
         ...
         */
        $buffer = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor();
        \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::copyToStream($body, $buffer, \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MIN_SIZE);
        // If body < 5MB, use PutObject with the buffer.
        if ($buffer->getSize() < \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MIN_SIZE) {
            $buffer->seek(0);
            $body = $buffer;
            return \false;
        }
        ...
        if ($body->isSeekable() && $body->getMetadata('uri') !== 'php://input') {
            ...
            // original stream.
            $buffer->seek(0);
            $body = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\AppendStream([$buffer, $body]);
        }
        return \true;
    }
}
