...
use WPStack_Connect_Vendor\Aws\Arn\S3\AccessPointArn;
 ...
 * when appropriate.
 */
class ObjectCopier implements \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_MULTIPART_THRESHOLD = \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MAX_SIZE;
    private $client;
    private $source;
    private $destination;
    private $acl;
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\S3\S3ClientInterface $client, array $source, array $destination, $acl = 'private', array $options = [])
    {
    ...
    public function promise()
    {
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            ...
            if ($objectStats['ContentLength'] > $this->options['mup_threshold']) {
                $mup = new \WPStack_Connect_Vendor\Aws\S3\MultipartCopy($this->client, $this->getSourcePath(), ['source_metadata' => $objectStats, 'acl' => $this->acl] + $this->destination + $this->options);
                (yield $mup->promise());
            } else {
    ...
    private function getSourcePath()
    {
        ...
        if (\WPStack_Connect_Vendor\Aws\Arn\ArnParser::isArn($this->source['Bucket'])) {
            try {
                new \WPStack_Connect_Vendor\Aws\Arn\S3\AccessPointArn($this->source['Bucket']);
