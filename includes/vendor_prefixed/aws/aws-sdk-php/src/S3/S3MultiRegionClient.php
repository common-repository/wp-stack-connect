...
use WPStack_Connect_Vendor\Aws\MultiRegionClient as BaseClient;
 ...
 */
class S3MultiRegionClient extends \WPStack_Connect_Vendor\Aws\MultiRegionClient implements \WPStack_Connect_Vendor\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var CacheInterface */
    private $cache;
    public static function getArguments()
    {
        ...
        unset($args['region']);
        return $args + ['bucket_region_cache' => ['type' => 'config', 'valid' => [\WPStack_Connect_Vendor\Aws\CacheInterface::class], 'doc' => 'Cache of regions in which given buckets are located.', 'default' => function () {
            return new \WPStack_Connect_Vendor\Aws\LruArrayCache();
        }], 'region' => $regionDef];
    }
    ...
    private function determineRegionMiddleware()
    {
        return function (callable $handler) {
            return function (\WPStack_Connect_Vendor\Aws\CommandInterface $command) use($handler) {
                $cacheKey = $this->getCacheKey($command['Bucket']);
                if (empty($command['@region']) && ($region = $this->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () use($handler, $command, $cacheKey) {
                    try {
                        (yield $handler($command));
                    } catch (\WPStack_Connect_Vendor\Aws\S3\Exception\PermanentRedirectException $e) {
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        if ($cached = $this->cache->get($cacheKey)) {
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($cached);
        }
