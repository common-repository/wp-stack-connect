...
trait MultipartUploadingTrait
{
     ...
     * @return UploadState
     */
    public static function getStateFromService(\WPStack_Connect_Vendor\Aws\S3\S3ClientInterface $client, $bucket, $key, $uploadId)
    {
        $state = new \WPStack_Connect_Vendor\Aws\Multipart\UploadState(['Bucket' => $bucket, 'Key' => $key, 'UploadId' => $uploadId]);
    ...
    protected abstract function extractETag(\WPStack_Connect_Vendor\Aws\ResultInterface $result);
    ...
    protected function determinePartSize()
    {
        ...
        $partSize = $this->getConfig()['part_size'] ?: \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MIN_SIZE;
        // Adjust the part size to be larger for known, x-large uploads.
        if ($sourceSize = $this->getSourceSize()) {
            $partSize = (int) \max($partSize, \ceil($sourceSize / \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MAX_NUM));
        }
        // Ensure that the part size follows the rules: 5 MB <= size <= 5 GB.
        if ($partSize < \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MIN_SIZE || $partSize > \WPStack_Connect_Vendor\Aws\S3\MultipartUploader::PART_MAX_SIZE) {
