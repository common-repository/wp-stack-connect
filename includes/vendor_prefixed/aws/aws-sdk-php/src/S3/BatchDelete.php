...
use WPStack_Connect_Vendor\Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 */
class BatchDelete implements \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface
{
    private $bucket;
     ...
     * @return BatchDelete
     */
    public static function fromListObjects(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, array $listObjectsParams, array $options = [])
    {
        $iter = $client->getPaginator('ListObjects', $listObjectsParams);
        $bucket = $listObjectsParams['Bucket'];
        $fn = function (\WPStack_Connect_Vendor\Aws\S3\BatchDelete $that) use($iter) {
            return $iter->each(function ($result) use($that) {
                ...
                return $promises ? \WPStack_Connect_Vendor\GuzzleHttp\Promise\Utils::all($promises) : null;
            });
        };
     ...
     * @return BatchDelete
     */
    public static function fromIterator(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, $bucket, \Iterator $iter, array $options = [])
    {
        $fn = function (\WPStack_Connect_Vendor\Aws\S3\BatchDelete $that) use($iter) {
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () use($that, $iter) {
                foreach ($iter as $obj) {
     ...
     */
    private function __construct(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, $bucket, callable $promiseFn, array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
    ...
    private function flushQueue()
    {
        ...
        return $this->client->executeAsync($command)->then(function ($result) {
            if (!empty($result['Errors'])) {
                throw new \WPStack_Connect_Vendor\Aws\S3\Exception\DeleteMultipleObjectsException($result['Deleted'] ?: [], $result['Errors']);
            }
            return $result;
        });
    }
    ...
    private function createPromise()
    {
        ...
        return $promise->then(function () use($cleanup) {
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($this->flushQueue())->then($cleanup);
