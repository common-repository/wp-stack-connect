...
class MockHandler implements \Countable
{
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof \WPStack_Connect_Vendor\Aws\ResultInterface || $value instanceof \Exception || \is_callable($value)) {
                $this->queue[] = $value;
            } else {
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($result instanceof \Exception) {
            $result = new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise($result);
        } else {
            ...
            $result = \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($result);
        }
