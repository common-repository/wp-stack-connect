...
namespace WPStack_Connect_Vendor\Aws\CloudTrail;
...
class LogRecordIterator implements \OuterIterator
{
     ...
     */
    public static function forTrail(\WPStack_Connect_Vendor\Aws\S3\S3Client $s3Client, \WPStack_Connect_Vendor\Aws\CloudTrail\CloudTrailClient $cloudTrailClient, array $options = [])
    {
        $logFileIterator = \WPStack_Connect_Vendor\Aws\CloudTrail\LogFileIterator::forTrail($s3Client, $cloudTrailClient, $options);
        return new self(new \WPStack_Connect_Vendor\Aws\CloudTrail\LogFileReader($s3Client), $logFileIterator);
    }
    /**
     * @param S3Client $s3Client
     ...
     * @return LogRecordIterator
     */
    public static function forBucket(\WPStack_Connect_Vendor\Aws\S3\S3Client $s3Client, $s3BucketName, array $options = [])
    {
        $logFileReader = new \WPStack_Connect_Vendor\Aws\CloudTrail\LogFileReader($s3Client);
        $iter = new \WPStack_Connect_Vendor\Aws\CloudTrail\LogFileIterator($s3Client, $s3BucketName, $options);
     ...
     * @return LogRecordIterator
     */
    public static function forFile(\WPStack_Connect_Vendor\Aws\S3\S3Client $s3Client, $s3BucketName, $s3ObjectKey)
    {
        $logFileReader = new \WPStack_Connect_Vendor\Aws\CloudTrail\LogFileReader($s3Client);
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\CloudTrail\LogFileReader $logFileReader, \Iterator $logFileIterator)
    {
