...
use WPStack_Connect_Vendor\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->client = isset($config['client']) ? $config['client'] : \WPStack_Connect_Vendor\Aws\default_http_handler();
    }
    /**
    ...
    {
        $this->attempts = 0;
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () use($previousCredentials) {
            ...
            while ($this->secureMode && \is_null($token)) {
                ...
                } catch (\WPStack_Connect_Vendor\GuzzleHttp\Exception\TransferException $e) {
                    if ($this->getExceptionStatusCode($e) === 500 && $previousCredentials instanceof \WPStack_Connect_Vendor\Aws\Credentials\Credentials) {
            ...
            while ($result == null) {
                ...
                } catch (\WPStack_Connect_Vendor\Aws\Exception\InvalidJsonException $e) {
                    $this->handleRetryableException($e, ['blacklist' => [401, 403]], $this->createErrorMessage('Invalid JSON response, retries exhausted'));
                } catch (\WPStack_Connect_Vendor\GuzzleHttp\Exception\TransferException $e) {
                    // 401 indicates insecure flow not supported, switch to
                    // attempting secure mode for subsequent calls
                    if (($this->getExceptionStatusCode($e) === 500 || \strpos($e->getMessage(), "cURL error 28") !== \false) && $previousCredentials instanceof \WPStack_Connect_Vendor\Aws\Credentials\Credentials) {
                        goto generateCredentials;
                    } else {
                        if (!empty($this->getExceptionStatusCode($e)) && $this->getExceptionStatusCode($e) === 401) {
                            $this->secureMode = \true;
            ...
            if (!isset($result)) {
                ...
                $credentials = new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], \strtotime($result['Expiration']));
            }
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        if (\strcasecmp($disabled, 'true') === 0) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException($this->createErrorMessage('EC2 metadata service access disabled'));
        }
        $fn = $this->client;
        $request = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Request($method, self::SERVER_URI . $url);
        $userAgent = 'aws-sdk-php/' . \WPStack_Connect_Vendor\Aws\Sdk::VERSION;
        if (\defined('HHVM_VERSION')) {
            $userAgent .= ' HHVM/' . HHVM_VERSION;
        }
        $userAgent .= ' ' . \WPStack_Connect_Vendor\Aws\default_user_agent();
        $request = $request->withHeader('User-Agent', $userAgent);
        foreach ($headers as $key => $value) {
            $request = $request->withHeader($key, $value);
        }
        return $fn($request, ['timeout' => $this->timeout])->then(function (\WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response) {
            return (string) $response->getBody();
        })->otherwise(function (array $reason) {
            $reason = $reason['exception'];
            if ($reason instanceof \WPStack_Connect_Vendor\GuzzleHttp\Exception\TransferException) {
                throw $reason;
            }
            $msg = $reason->getMessage();
            throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException($this->createErrorMessage($msg));
        });
    }
    private function handleRetryableException(\Exception $e, $retryOptions, $message)
    {
        ...
        if ($isRetryable && $this->attempts < $this->retries) {
            ...
            throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException($message);
        }
    }
    ...
    private function decodeResult($response)
    {
        ...
        if (\json_last_error() > 0) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\InvalidJsonException();
        }
        if ($result['Code'] !== 'Success') {
            throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException('Unexpected instance profile ' . 'response code: ' . $result['Code']);
        }
        return $result;
    }
}
