...
namespace WPStack_Connect_Vendor\Aws\Credentials;
...
use WPStack_Connect_Vendor\Aws\Exception\CredentialsException;
use WPStack_Connect_Vendor\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['credentials']) && $config['credentials'] instanceof \WPStack_Connect_Vendor\Aws\CacheInterface) {
        ...
        return self::memoize(\call_user_func_array([\WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::class, 'chain'], \array_values($defaultChain)));
    }
    /**
     ...
     * @return callable
     */
    public static function fromCredentials(\WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($creds);
    ...
    public static function chain()
    {
        ...
        return function ($previousCreds = null) use($links) {
            ...
            while ($next = \array_shift($links)) {
                if ($next instanceof \WPStack_Connect_Vendor\Aws\Credentials\InstanceProfileProvider && $previousCreds instanceof \WPStack_Connect_Vendor\Aws\Credentials\Credentials) {
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
                ...
                $result = null;
                return new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise($reason);
            });
        };
    }
    /**
     ...
     * @return callable
     */
    public static function cache(callable $provider, \WPStack_Connect_Vendor\Aws\CacheInterface $cache, $cacheKey = null)
    {
        $cacheKey = $cacheKey ?: 'aws_cached_credentials';
        return function () use($provider, $cache, $cacheKey) {
            $found = $cache->get($cacheKey);
            if ($found instanceof \WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface && !$found->isExpired()) {
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($found);
            }
            return $provider()->then(function (\WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($key, $secret, \getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    public static function sso($ssoProfileName = 'default', $filename = null, $config = [])
    {
        ...
        return function () use($ssoProfileName, $filename, $config) {
            ...
            if (!empty($ssoProfile['sso_session'])) {
                return \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::getSsoCredentials($profiles, $ssoProfileName, $filename, $config);
            } else {
                return \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::getSsoCredentialsLegacy($profiles, $ssoProfileName, $filename, $config);
            }
        };
    }
    /**
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \WPStack_Connect_Vendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            $profileName = \getenv(self::ENV_PROFILE) ?: 'default';
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \WPStack_Connect_Vendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPStack_Connect_Vendor\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
            if ($data === \false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \WPStack_Connect_Vendor\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \WPStack_Connect_Vendor\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient, $config = [])
    {
        ...
        if (empty($stsClient)) {
            ...
            if (!empty($roleProfile['source_profile'])) {
                $sourceCredentials = \call_user_func(\WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            } else {
                $sourceCredentials = self::getCredentialsFromSource($profileName, $filename);
            }
            $stsClient = new \WPStack_Connect_Vendor\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $credentials = $stsClient->createCredentials($result);
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($credentials);
    }
    /**
    ...
    {
        $profileData = \WPStack_Connect_Vendor\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && \getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \WPStack_Connect_Vendor\Aws\parse_ini_file($configFilename, \true, \INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (\file_exists($credFile)) {
            $profiles = \WPStack_Connect_Vendor\Aws\parse_ini_file($credFile, \true, \INI_SCANNER_RAW);
        }
        if (\file_exists($configFile)) {
            $configProfileData = \WPStack_Connect_Vendor\Aws\parse_ini_file($configFile, \true, \INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = \str_replace('profile ', '', $name);
    ...
    public static function getCredentialsFromSource($profileName = '', $filename = '', $config = [])
    {
        ...
        switch ($credentialSource) {
            ...
            case 'EcsContainer':
                ...
                break;
            default:
                throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException("Invalid credential_source found in config file: {$credentialSource}. Valid inputs " . "include Environment, Ec2InstanceMetadata, and EcsContainer.");
        }
        ...
        return function () use($credentialsResult) {
            return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($credentialsResult);
        };
    }
    private static function reject($msg)
    {
        return new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise(new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException($msg));
    }
    /**
     * @param $filename
     * @return string
    ...
    public static function shouldUseEcs()
    {
        ...
        return !empty(\getenv(\WPStack_Connect_Vendor\Aws\Credentials\EcsCredentialProvider::ENV_URI)) || !empty($_SERVER[\WPStack_Connect_Vendor\Aws\Credentials\EcsCredentialProvider::ENV_URI]) || !empty(\getenv(\WPStack_Connect_Vendor\Aws\Credentials\EcsCredentialProvider::ENV_FULL_URI)) || !empty($_SERVER[\WPStack_Connect_Vendor\Aws\Credentials\EcsCredentialProvider::ENV_FULL_URI]);
    }
    /**
     * @param $profiles
    ...
    private static function getSsoCredentials($profiles, $ssoProfileName, $filename, $config)
    {
        if (empty($config['ssoOidcClient'])) {
            ...
            $ssoOidcClient = new \WPStack_Connect_Vendor\Aws\SSOOIDC\SSOOIDCClient(['region' => $ssoSession['sso_region'], 'version' => '2019-06-10', 'credentials' => \false]);
        } else {
            $ssoOidcClient = $config['ssoClient'];
        }
        $tokenPromise = new \WPStack_Connect_Vendor\Aws\Token\SsoTokenProvider($ssoProfileName, $filename, $ssoOidcClient);
        $token = $tokenPromise()->wait();
        $ssoCredentials = \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::getCredentialsFromSsoService($ssoProfile, $ssoSession['sso_region'], $token->getToken(), $config);
        $expiration = $ssoCredentials['expiration'];
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($ssoCredentials['accessKeyId'], $ssoCredentials['secretAccessKey'], $ssoCredentials['sessionToken'], $expiration));
    }
    /**
     * @param $profiles
     * @param $ssoProfileName
    ...
    private static function getSsoCredentialsLegacy($profiles, $ssoProfileName, $filename, $config)
    {
        ...
        try {
            $expiration = (new \WPStack_Connect_Vendor\Aws\Api\DateTimeResult($tokenData['expiresAt']))->getTimestamp();
        } catch (\Exception $e) {
        ...
        $ssoCredentials = \WPStack_Connect_Vendor\Aws\Credentials\CredentialProvider::getCredentialsFromSsoService($ssoProfile, $ssoProfile['sso_region'], $tokenData['accessToken'], $config);
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStack_Connect_Vendor\Aws\Credentials\Credentials($ssoCredentials['accessKeyId'], $ssoCredentials['secretAccessKey'], $ssoCredentials['sessionToken'], $expiration));
    }
    /**
    ...
    private static function getCredentialsFromSsoService($ssoProfile, $clientRegion, $accessToken, $config)
    {
        if (empty($config['ssoClient'])) {
            $ssoClient = new \WPStack_Connect_Vendor\Aws\SSO\SSOClient(['region' => $clientRegion, 'version' => '2019-06-10', 'credentials' => \false]);
        } else {
