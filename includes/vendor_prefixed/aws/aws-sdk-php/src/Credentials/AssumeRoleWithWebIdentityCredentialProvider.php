...
class AssumeRoleWithWebIdentityCredentialProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['client'])) {
            ...
            $this->client = new \WPStack_Connect_Vendor\Aws\Sts\StsClient(['credentials' => \false, 'region' => $region, 'version' => 'latest']);
        }
    }
    /**
    ...
    public function __invoke()
    {
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            ...
            while ($result == null) {
                try {
                    ...
                    if (\false === $token) {
                        ...
                        if (!@\is_readable($this->tokenFile)) {
                            throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException("Unreadable tokenfile at location {$this->tokenFile}");
                        }
                        $token = @\file_get_contents($this->tokenFile);
                    }
                    if (empty($token)) {
                        ...
                        throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException("InvalidIdentityToken from file: {$this->tokenFile}");
                    }
                } catch (\Exception $exception) {
                    throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException("Error reading WebIdentityTokenFile from " . $this->tokenFile, 0, $exception);
                }
                ...
                try {
                ...
                } catch (\WPStack_Connect_Vendor\Aws\Exception\AwsException $e) {
                    if ($e->getAwsErrorCode() == 'InvalidIdentityToken') {
                        if ($this->authenticationAttempts < $this->retries) {
                            ...
                            throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException("InvalidIdentityToken, retries exhausted");
                        }
                    } else {
                        throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException("Error assuming role from web identity credentials", 0, $e);
                    }
                } catch (\Exception $e) {
                    throw new \WPStack_Connect_Vendor\Aws\Exception\CredentialsException("Error retrieving web identity credentials: " . $e->getMessage() . " (" . $e->getCode() . ")");
                }
