 ...
 * @internal
 */
class OutpostsBucketArn extends \WPStack_Connect_Vendor\Aws\Arn\Arn implements \WPStack_Connect_Vendor\Aws\Arn\S3\BucketArnInterface, \WPStack_Connect_Vendor\Aws\Arn\S3\OutpostsArnInterface
{
    ...
    public static function validate(array $data)
    {
        ...
        if ($data['service'] !== 's3-outposts') {
            throw new \WPStack_Connect_Vendor\Aws\Arn\Exception\InvalidArnException("The 3rd component of an S3 Outposts" . " bucket ARN represents the service and must be 's3-outposts'.");
        }
        ...
        if ($data['resource_type'] !== 'outpost') {
            throw new \WPStack_Connect_Vendor\Aws\Arn\Exception\InvalidArnException("The 6th component of an S3 Outposts" . " bucket ARN represents the resource type and must be" . " 'outpost'.");
        }
        if (!self::isValidHostLabel($data['outpost_id'])) {
            throw new \WPStack_Connect_Vendor\Aws\Arn\Exception\InvalidArnException("The 7th component of an S3 Outposts" . " bucket ARN is required, represents the outpost ID, and" . " must be a valid host label.");
        }
        if ($data['bucket_label'] !== 'bucket') {
            throw new \WPStack_Connect_Vendor\Aws\Arn\Exception\InvalidArnException("The 8th component of an S3 Outposts" . " bucket ARN must be 'bucket'");
        }
        if (empty($data['bucket_name'])) {
            throw new \WPStack_Connect_Vendor\Aws\Arn\Exception\InvalidArnException("The 9th component of an S3 Outposts" . " bucket ARN represents the bucket name and must not be empty.");
        }
    }
}
