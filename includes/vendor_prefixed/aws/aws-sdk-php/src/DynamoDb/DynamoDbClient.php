...
use WPStack_Connect_Vendor\Aws\Api\Parser\Crc32ValidatingParser;
 ...
 */
class DynamoDbClient extends \WPStack_Connect_Vendor\Aws\AwsClient
{
    ...
    {
        $handler = \WPStack_Connect_Vendor\Aws\DynamoDb\SessionHandler::fromClient($this, $config);
        $handler->register();
        return $handler;
    }
    /** @internal */
    public static function _applyRetryConfig($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \WPStack_Connect_Vendor\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $list->appendSign(\WPStack_Connect_Vendor\Aws\Middleware::retry(\WPStack_Connect_Vendor\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1, ['error_codes' => ['TransactionInProgressException']]), function ($retries) {
                    return $retries ? \WPStack_Connect_Vendor\Aws\RetryMiddleware::exponentialDelay($retries) / 2 : 0;
                }, isset($args['stats']['retries']) ? (bool) $args['stats']['retries'] : \false), 'retry');
            } else {
                $list->appendSign(\WPStack_Connect_Vendor\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'transient_error_codes' => ['TransactionInProgressException']]), 'retry');
            }
        }
    }
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \WPStack_Connect_Vendor\Aws\HandlerList $list)
    {
        ...
        $args['parser'] = new \WPStack_Connect_Vendor\Aws\Api\Parser\Crc32ValidatingParser($args['parser']);
    }
}
