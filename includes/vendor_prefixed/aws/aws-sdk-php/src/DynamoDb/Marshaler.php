...
class Marshaler
{
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\DynamoDb\BinaryValue($value);
    }
    /**
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\DynamoDb\NumberValue($value);
    }
    /**
    ...
    {
        return new \WPStack_Connect_Vendor\Aws\DynamoDb\SetValue($values);
    }
    /**
    ...
    public function marshalValue($value)
    {
        ...
        if ($type === 'integer' || $type === 'double' || $value instanceof \WPStack_Connect_Vendor\Aws\DynamoDb\NumberValue) {
        ...
        // Handle set values.
        if ($value instanceof \WPStack_Connect_Vendor\Aws\DynamoDb\SetValue) {
            if (\count($value) === 0) {
        ...
        if (\is_resource($value) || $value instanceof \WPStack_Connect_Vendor\Psr\Http\Message\StreamInterface) {
            $value = $this->binary($value);
        }
        if ($value instanceof \WPStack_Connect_Vendor\Aws\DynamoDb\BinaryValue) {
    ...
    public function unmarshalValue(array $value, $mapAsObject = \false)
    {
        ...
        switch ($type) {
            ...
            case 'BS':
                ...
                return new \WPStack_Connect_Vendor\Aws\DynamoDb\SetValue($value);
        }
