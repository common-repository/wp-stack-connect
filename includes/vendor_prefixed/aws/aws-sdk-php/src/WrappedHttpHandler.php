...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = \WPStack_Connect_Vendor\Aws\Exception\AwsException::class, $collectStats = \false)
    {
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \WPStack_Connect_Vendor\GuzzleHttp\Promise\Create::promiseFor($fn($request, $options))->then(function (\WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \WPStack_Connect_Vendor\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\WPStack_Connect_Vendor\Aws\CommandInterface $command, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, \WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \WPStack_Connect_Vendor\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, \WPStack_Connect_Vendor\Aws\CommandInterface $command, array $stats)
    {
