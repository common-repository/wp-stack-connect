 ...
 */
class CommandPool implements \WPStack_Connect_Vendor\GuzzleHttp\Promise\PromisorInterface
{
    /** @var EachPromise */
    private $each;
    /**
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        ...
        $mapFn = function ($commands) use($client, $before, $config) {
            foreach ($commands as $key => $command) {
                if (!$command instanceof \WPStack_Connect_Vendor\Aws\CommandInterface) {
        ...
        $this->each = new \WPStack_Connect_Vendor\GuzzleHttp\Promise\EachPromise($mapFn($commands), $config);
    }
    /**
     ...
     */
    public static function batch(\WPStack_Connect_Vendor\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        $results = [];
