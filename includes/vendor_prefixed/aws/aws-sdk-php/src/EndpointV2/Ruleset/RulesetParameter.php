...
class RulesetParameter
{
    ...
    public function __construct($name, array $definition)
    {
        ...
        if ($this->isValidType($type)) {
            ...
            throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Unknown parameter type ' . "`{$type}`" . '. Parameters must be of type `String` or `Boolean`.');
        }
        $this->name = $name;
    ...
    public function validateInputParam($inputParam)
    {
        ...
        if ($typeMap[$this->type]($inputParam) === \false) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException("Input parameter `{$this->name}` is the wrong type. Must be a {$this->type}.");
        }
        if ($this->deprecated) {
