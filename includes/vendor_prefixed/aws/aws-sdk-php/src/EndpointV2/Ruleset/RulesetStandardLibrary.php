...
namespace WPStack_Connect_Vendor\Aws\EndpointV2\Ruleset;
...
class RulesetStandardLibrary
{
    ...
    {
        if (!\is_string($input)) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Input passed to `substring` must be `string`.');
        }
    ...
    public function stringEquals($string1, $string2)
    {
        if (!\is_string($string1) || !\is_string($string2)) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Values passed to StringEquals must be `string`.');
        }
    ...
    {
        if (!\is_string($region)) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException('Value passed to `partition` must be `string`.');
        }
    ...
    public function callFunction($funcCondition, &$inputParameters)
    {
        ...
        $result = \call_user_func_array([\WPStack_Connect_Vendor\Aws\EndpointV2\Ruleset\RulesetStandardLibrary::class, $funcName], $funcArgs);
        if (isset($funcCondition['assign'])) {
            $assign = $funcCondition['assign'];
            if (isset($inputParameters[$assign])) {
                throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException("Assignment `{$assign}` already exists in input parameters" . " or has already been assigned by an endpoint rule and cannot be overwritten.");
            }
            ...
            if (\count($parts) > 1) {
                ...
                foreach ($parts as $part) {
                    if (!isset($resolvedValue[$part])) {
                        throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedEndpointException($notFoundMessage);
                    }
