...
use WPStack_Connect_Vendor\Aws\EndpointV2\Rule\RuleCreator;
/**
...
class Ruleset
{
    ...
    public function __construct(array $ruleset, array $partitions)
    {
        ...
        $this->standardLibrary = new \WPStack_Connect_Vendor\Aws\EndpointV2\Ruleset\RulesetStandardLibrary($partitions);
    }
    /**
     * @return mixed
     */
    ...
    private function createParameters(array $parameters)
    {
        ...
        foreach ($parameters as $name => $definition) {
            $parameterList[$name] = new \WPStack_Connect_Vendor\Aws\EndpointV2\Ruleset\RulesetParameter($name, $definition);
        }
    ...
    private function createRules(array $rules)
    {
        ...
        foreach ($rules as $rule) {
            $ruleObj = \WPStack_Connect_Vendor\Aws\EndpointV2\Rule\RuleCreator::create($rule['type'], $rule);
            $rulesList[] = $ruleObj;
        }
        return $rulesList;
    }
