...
use WPStack_Connect_Vendor\Aws\Endpoint\PartitionInterface;
...
class MultiRegionClient implements \WPStack_Connect_Vendor\Aws\AwsClientInterface
{
    use AwsClientTrait;
    ...
    {
        $args = \array_intersect_key(\WPStack_Connect_Vendor\Aws\ClientResolver::getDefaultArguments(), ['service' => \true, 'region' => \true]);
        ...
        return $args + ['client_factory' => ['type' => 'config', 'valid' => ['callable'], 'doc' => 'A callable that takes an array of client' . ' configuration arguments and returns a regionalized' . ' client.', 'required' => \true, 'internal' => \true, 'default' => function (array $args) {
        ...
        }], 'partition' => ['type' => 'config', 'valid' => ['string', \WPStack_Connect_Vendor\Aws\Endpoint\PartitionInterface::class], 'doc' => 'AWS partition to connect to. Valid partitions' . ' include "aws," "aws-cn," and "aws-us-gov." Used to' . ' restrict the scope of the mapRegions method.', 'default' => function (array $args) {
            $region = isset($args['region']) ? $args['region'] : '';
            return \WPStack_Connect_Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($region, $args['service']);
        }, 'fn' => function ($value, array &$args) {
            if (\is_string($value)) {
                $value = \WPStack_Connect_Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartitionByName($value);
            }
            if (!$value instanceof \WPStack_Connect_Vendor\Aws\Endpoint\PartitionInterface) {
            ...
            $ruleset = \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointDefinitionProvider::getEndpointRuleset($args['service'], isset($args['version']) ? $args['version'] : 'latest');
            $partitions = \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointDefinitionProvider::getPartitions();
            $args['endpoint_provider'] = new \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointProviderV2($ruleset, $partitions);
        }]];
    }
    /**
    ...
    public function __construct(array $args = [])
    {
        ...
        $this->handlerList = new \WPStack_Connect_Vendor\Aws\HandlerList(function (\WPStack_Connect_Vendor\Aws\CommandInterface $command) {
        ...
        $resolver = new \WPStack_Connect_Vendor\Aws\ClientResolver($argDefinitions);
