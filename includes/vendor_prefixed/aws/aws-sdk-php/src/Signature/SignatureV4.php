...
use WPStack_Connect_Vendor\AWS\CRT\Auth\Signable;
use WPStack_Connect_Vendor\AWS\CRT\Auth\SignatureType;
use WPStack_Connect_Vendor\AWS\CRT\Auth\Signing;
use WPStack_Connect_Vendor\AWS\CRT\Auth\SigningAlgorithm;
use WPStack_Connect_Vendor\AWS\CRT\Auth\SigningConfigAWS;
use WPStack_Connect_Vendor\AWS\CRT\Auth\StaticCredentialsProvider;
use WPStack_Connect_Vendor\AWS\CRT\HTTP\Request;
use WPStack_Connect_Vendor\Aws\Exception\CommonRuntimeException;
use WPStack_Connect_Vendor\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \WPStack_Connect_Vendor\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
     ...
     * {@inheritdoc}
     */
    public function signRequest(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, \WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $credentials, $signingService = null)
    {
     ...
     */
    public static function convertPostToGet(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, $additionalQueryParams = "")
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    protected function getPresignedPayload(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, \WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Query::parse($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\WPStack_Connect_Vendor\GuzzleHttp\Psr7\Query::build($req['query']));
        }
        return new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
    private function verifyCRTLoaded()
    {
        if (!\extension_loaded('awscrt')) {
            throw new \WPStack_Connect_Vendor\Aws\Exception\CommonRuntimeException("AWS Common Runtime for PHP is required to use Signature V4A" . ".  Please install it using the instructions found at" . " https://github.com/aws/aws-sdk-php/blob/master/CRT_INSTRUCTIONS.md");
        }
    }
    private function createCRTStaticCredentialsProvider($credentials)
    {
        return new \WPStack_Connect_Vendor\AWS\CRT\Auth\StaticCredentialsProvider(['access_key_id' => $credentials->getAccessKeyId(), 'secret_access_key' => $credentials->getSecretKey(), 'session_token' => $credentials->getSecurityToken()]);
    }
    ...
    {
        return new \WPStack_Connect_Vendor\AWS\CRT\HTTP\Request(
            $request->getMethod(),
     ...
     * @return RequestInterface
     */
    protected function signWithV4a(\WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $credentials, \WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, $signingService)
    {
        $this->verifyCRTLoaded();
        $credentials_provider = $this->createCRTStaticCredentialsProvider($credentials);
        $signingConfig = new \WPStack_Connect_Vendor\AWS\CRT\Auth\SigningConfigAWS(['algorithm' => \WPStack_Connect_Vendor\AWS\CRT\Auth\SigningAlgorithm::SIGv4_ASYMMETRIC, 'signature_type' => \WPStack_Connect_Vendor\AWS\CRT\Auth\SignatureType::HTTP_REQUEST_HEADERS, 'credentials_provider' => $credentials_provider, 'signed_body_value' => $this->getPayload($request), 'region' => "*", 'service' => $signingService, 'date' => \time()]);
        $removedIllegalHeaders = $this->removeIllegalV4aHeaders($request);
        $http_request = $this->CRTRequestFromGuzzleRequest($request);
        \WPStack_Connect_Vendor\AWS\CRT\Auth\Signing::signRequestAws(\WPStack_Connect_Vendor\AWS\CRT\Auth\Signable::fromHttpRequest($http_request), $signingConfig, function ($signing_result, $error_code) use(&$http_request) {
    ...
    protected function presignWithV4a(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, \WPStack_Connect_Vendor\Aws\Credentials\CredentialsInterface $credentials, $expires)
    {
        $this->verifyCRTLoaded();
        $credentials_provider = $this->createCRTStaticCredentialsProvider($credentials);
        $signingConfig = new \WPStack_Connect_Vendor\AWS\CRT\Auth\SigningConfigAWS(['algorithm' => \WPStack_Connect_Vendor\AWS\CRT\Auth\SigningAlgorithm::SIGv4_ASYMMETRIC, 'signature_type' => \WPStack_Connect_Vendor\AWS\CRT\Auth\SignatureType::HTTP_REQUEST_QUERY_PARAMS, 'credentials_provider' => $credentials_provider, 'signed_body_value' => $this->getPresignedPayload($request), 'region' => "*", 'service' => $this->service, 'date' => \time(), 'expiration_in_seconds' => $expires]);
        ...
        return $request->withUri(new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($http_request->pathAndQuery()));
    }
}
