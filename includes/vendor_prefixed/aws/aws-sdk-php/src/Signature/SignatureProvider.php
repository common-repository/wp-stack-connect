...
use WPStack_Connect_Vendor\Aws\Exception\UnresolvedSignatureException;
use WPStack_Connect_Vendor\Aws\Token\BearerTokenAuthorization;
/**
 * Signature providers.
 *
...
class SignatureProvider
{
    ...
    public static function resolve(callable $provider, $version, $service, $region)
    {
        ...
        if ($result instanceof \WPStack_Connect_Vendor\Aws\Signature\SignatureInterface || $result instanceof \WPStack_Connect_Vendor\Aws\Token\BearerTokenAuthorization) {
            return $result;
        }
        throw new \WPStack_Connect_Vendor\Aws\Exception\UnresolvedSignatureException("Unable to resolve a signature for {$version}/{$service}/{$region}.\n" . "Valid signature versions include v4 and anonymous.");
    }
    /**
    ...
    public static function version()
    {
        return function ($version, $service, $region) {
            switch ($version) {
                case 's3v4':
                case 'v4':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \WPStack_Connect_Vendor\Aws\Signature\S3SignatureV4($service, $region) : new \WPStack_Connect_Vendor\Aws\Signature\SignatureV4($service, $region);
                case 'v4a':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \WPStack_Connect_Vendor\Aws\Signature\S3SignatureV4($service, $region, ['use_v4a' => \true]) : new \WPStack_Connect_Vendor\Aws\Signature\SignatureV4($service, $region, ['use_v4a' => \true]);
                case 'v4-unsigned-body':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \WPStack_Connect_Vendor\Aws\Signature\S3SignatureV4($service, $region, ['unsigned-body' => 'true']) : new \WPStack_Connect_Vendor\Aws\Signature\SignatureV4($service, $region, ['unsigned-body' => 'true']);
                case 'bearer':
                    return new \WPStack_Connect_Vendor\Aws\Token\BearerTokenAuthorization();
                case 'anonymous':
                    return new \WPStack_Connect_Vendor\Aws\Signature\AnonymousSignature();
                default:
                    return null;
            }
        };
