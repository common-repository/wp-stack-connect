...
namespace WPStack_Connect_Vendor\Aws\CloudFront;
...
class UrlSigner
{
    ...
    {
        $this->signer = new \WPStack_Connect_Vendor\Aws\CloudFront\Signer($keyPairId, $privateKey);
    }
    /**
    ...
    public function getSignedUrl($url, $expires = null, $policy = null)
    {
        ...
        $uri = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($scheme . '://' . $urlSections[1]);
        $query = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Query::parse($uri->getQuery(), \PHP_QUERY_RFC3986);
    ...
    private function createRtmpUrl(\WPStack_Connect_Vendor\Psr\Http\Message\UriInterface $uri)
    {
