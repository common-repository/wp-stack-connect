...
use WPStack_Connect_Vendor\Aws\Api\Operation;
...
use WPStack_Connect_Vendor\GuzzleHttp\Psr7\UriResolver;
...
abstract class RestSerializer
{
     ...
     */
    public function __construct(\WPStack_Connect_Vendor\Aws\Api\Service $api, $endpoint)
    {
        $this->api = $api;
        $this->endpoint = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::uriFor($endpoint);
    }
    /**
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\CommandInterface $command, $endpointProvider = null, $clientArgs = null)
    {
        ...
        if ($endpointProvider instanceof \WPStack_Connect_Vendor\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $this->endpoint = new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($this->endpoint);
        }
        $uri = $this->buildEndpoint($operation, $commandArgs, $opts);
        return new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Request($operation['http']['method'], $uri, $headers, isset($opts['body']) ? $opts['body'] : null);
    }
    /**
     ...
     */
    protected abstract function payload(\WPStack_Connect_Vendor\Aws\Api\StructureShape $member, array $value, array &$opts);
    private function serialize(\WPStack_Connect_Vendor\Aws\Api\Operation $operation, array $args)
    {
        $opts = [];
    ...
    private function applyPayload(\WPStack_Connect_Vendor\Aws\Api\StructureShape $input, $name, array $args, array &$opts)
    {
        if (!isset($args[$name])) {
            return;
        }
        ...
        if ($m['streaming'] || ($m['type'] == 'string' || $m['type'] == 'blob')) {
            ...
            $opts['body'] = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Utils::streamFor($args[$name]);
            return;
        }
        $this->payload($m, $args[$name], $opts);
    }
    private function applyHeader($name, \WPStack_Connect_Vendor\Aws\Api\Shape $member, $value, array &$opts)
    {
        if ($member->getType() === 'timestamp') {
            $timestampFormat = !empty($member['timestampFormat']) ? $member['timestampFormat'] : 'rfc822';
            $value = \WPStack_Connect_Vendor\Aws\Api\TimestampShape::format($value, $timestampFormat);
     ...
     */
    private function applyHeaderMap($name, \WPStack_Connect_Vendor\Aws\Api\Shape $member, array $value, array &$opts)
    {
    ...
    private function applyQuery($name, \WPStack_Connect_Vendor\Aws\Api\Shape $member, $value, array &$opts)
    {
        if ($member instanceof \WPStack_Connect_Vendor\Aws\Api\MapShape) {
    ...
    private function buildEndpoint(\WPStack_Connect_Vendor\Aws\Api\Operation $operation, array $args, array $opts)
    {
        ...
        if ($this->api->isModifiedModel() && \strpos($relative, '//') === 0) {
            return new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($this->endpoint . $relative);
        }
        // Expand path place holders using Amazon's slightly different URI
        // template syntax.
        return \WPStack_Connect_Vendor\GuzzleHttp\Psr7\UriResolver::resolve($this->endpoint, new \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Uri($relative));
    }
    /**
     * @param StructureShape $input
     */
    private function hasPayloadParam(\WPStack_Connect_Vendor\Aws\Api\StructureShape $input, $payload)
    {
        if ($payload) {
    ...
    {
        $append = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Query::build($query);
