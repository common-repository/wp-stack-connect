...
class QueryParamBuilder
{
    ...
    public function __invoke(\WPStack_Connect_Vendor\Aws\Api\StructureShape $shape, array $params)
    {
        if (!$this->methods) {
    ...
    protected function format(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
    ...
    protected function format_structure(\WPStack_Connect_Vendor\Aws\Api\StructureShape $shape, array $value, $prefix, &$query)
    {
        if ($prefix) {
            $prefix .= '.';
        }
    ...
    protected function format_map(\WPStack_Connect_Vendor\Aws\Api\MapShape $shape, array $value, $prefix, array &$query)
    {
        $vals = $shape->getValue();
        $keys = $shape->getKey();
        if (!$this->isFlat($shape)) {
    ...
    protected function format_blob(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
        $query[$prefix] = \base64_encode($value);
    }
    protected function format_timestamp(\WPStack_Connect_Vendor\Aws\Api\TimestampShape $shape, $value, $prefix, array &$query)
    {
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $query[$prefix] = \WPStack_Connect_Vendor\Aws\Api\TimestampShape::format($value, $timestampFormat);
    }
    protected function format_boolean(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
