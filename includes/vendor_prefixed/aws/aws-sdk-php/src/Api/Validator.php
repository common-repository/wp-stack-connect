...
class Validator
{
     ...
     */
    public function validate($name, \WPStack_Connect_Vendor\Aws\Api\Shape $shape, array $input)
    {
    ...
    private function dispatch(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value)
    {
    ...
    private function check_structure(\WPStack_Connect_Vendor\Aws\Api\StructureShape $shape, $value)
    {
    ...
    private function check_list(\WPStack_Connect_Vendor\Aws\Api\ListShape $shape, $value)
    {
        if (!\is_array($value)) {
            $this->addError('must be an array. Found ' . \WPStack_Connect_Vendor\Aws\describe_type($value));
            return;
        }
    ...
    private function check_map(\WPStack_Connect_Vendor\Aws\Api\MapShape $shape, $value)
    {
    ...
    private function check_blob(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value)
    {
        static $valid = ['string' => \true, 'integer' => \true, 'double' => \true, 'resource' => \true];
        $type = \gettype($value);
        if (!isset($valid[$type])) {
            if ($type != 'object' || !\method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \WPStack_Connect_Vendor\Aws\describe_type($value));
            }
        }
    }
    private function check_numeric(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value)
    {
        if (!\is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \WPStack_Connect_Vendor\Aws\describe_type($value));
            return;
        }
        $this->validateRange($shape, $value, "numeric value");
    }
    private function check_boolean(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value)
    {
        if (!\is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \WPStack_Connect_Vendor\Aws\describe_type($value));
        }
    }
    private function check_string(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \WPStack_Connect_Vendor\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \WPStack_Connect_Vendor\Aws\describe_type($value));
            return;
        }
    ...
    private function validateRange(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $length, $descriptor)
    {
    ...
    private function checkAssociativeArray($value)
    {
        ...
        if (!$isAssociative) {
            $this->addError('must be an associative array. Found ' . \WPStack_Connect_Vendor\Aws\describe_type($value));
            return \false;
        }
        return \true;
    }
