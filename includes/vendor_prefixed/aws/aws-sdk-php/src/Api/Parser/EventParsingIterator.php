...
use Iterator;
use WPStack_Connect_Vendor\Aws\Exception\EventStreamDataException;
...
class EventParsingIterator implements \Iterator
{
    ...
    /** @var AbstractParser */
    private $parser;
    public function __construct(\WPStack_Connect_Vendor\Psr\Http\Message\StreamInterface $stream, \WPStack_Connect_Vendor\Aws\Api\StructureShape $shape, \WPStack_Connect_Vendor\Aws\Api\Parser\AbstractParser $parser)
    {
        $this->decodingIterator = new \WPStack_Connect_Vendor\Aws\Api\Parser\DecodingEventStreamIterator($stream);
        $this->shape = $shape;
        $this->parser = $parser;
    }
    ...
    private function parseEvent(array $event)
    {
        if (!empty($event['headers'][':message-type'])) {
            ...
            if ($event['headers'][':message-type'] !== 'event') {
                throw new \WPStack_Connect_Vendor\Aws\Api\Parser\Exception\ParserException('Failed to parse unknown message type.');
            }
        }
        if (empty($event['headers'][':event-type'])) {
            throw new \WPStack_Connect_Vendor\Aws\Api\Parser\Exception\ParserException('Failed to parse without event type.');
        }
    ...
    {
        throw new \WPStack_Connect_Vendor\Aws\Exception\EventStreamDataException($event['headers'][':error-code'], $event['headers'][':error-message']);
    }
}
