...
class JsonParser
{
    public function parse(\WPStack_Connect_Vendor\Aws\Api\Shape $shape, $value)
    {
        if ($value === null) {
            return $value;
        }
        switch ($shape['type']) {
            ...
            case 'map':
                ...
                return $target;
            case 'timestamp':
                return \WPStack_Connect_Vendor\Aws\Api\DateTimeResult::fromTimestamp($value, !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : null);
            case 'blob':
