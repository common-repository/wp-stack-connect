...
use WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\OAuthSignature;
use WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Utils;
if (!\class_exists(\WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\BitBucketApi::class, \false)) {
    class BitBucketApi extends \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Api
    {
        /**
         * @var OAuthSignature
         */
        ...
        public function getBranch($branchName)
        {
            ...
            return new \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Reference(array('name' => $ref, 'updated' => $branch->target->date, 'downloadUrl' => $this->getDownloadUrl($branch->name)));
        }
        /**
         * Get a specific tag.
         *
        ...
        public function getTag($tagName)
        {
            ...
            return new \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Reference(array('name' => $tag->name, 'version' => \ltrim($tag->name, 'v'), 'updated' => $tag->target->date, 'downloadUrl' => $this->getDownloadUrl($tag->name)));
        }
        /**
        ...
        {
            $url = \ltrim($url, '/');
            $isSrcResource = \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Utils::startsWith($url, 'src/');
            ...
            $error = new \WPStack_Connect_Vendor\WP_Error('puc-bitbucket-http-error', \sprintf('BitBucket API error. Base URL: "%s",  HTTP status code: %d.', $baseUrl, $code));
        ...
        public function setAuthentication($credentials)
        {
            ...
            if (!empty($credentials) && !empty($credentials['consumer_key'])) {
                $this->oauth = new \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\OAuthSignature($credentials['consumer_key'], $credentials['consumer_secret']);
            } else {
                $this->oauth = null;
            }
        }
