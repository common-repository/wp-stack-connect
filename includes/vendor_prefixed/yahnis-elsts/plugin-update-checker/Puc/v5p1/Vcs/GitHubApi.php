...
use WPStack_Connect_Vendor\Parsedown;
if (!\class_exists(\WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\GitHubApi::class, \false)) {
    class GitHubApi extends \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Api
    {
        ...
        public function getLatestRelease()
        {
            ...
            foreach ($foundReleases as $release) {
                ...
                $reference = new \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Reference(array('name' => $release->tag_name, 'version' => $versionNumber, 'downloadUrl' => $release->zipball_url, 'updated' => $release->created_at, 'apiResponse' => $release));
                ...
                if ($this->releaseAssetsEnabled) {
                    ...
                    if (!empty($matchingAssets)) {
                        ...
                        if ($this->releaseAssetPreference === \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Api::REQUIRE_RELEASE_ASSETS) {
                ...
                if (!empty($release->body)) {
                    $reference->changelog = \WPStack_Connect_Vendor\Parsedown::instance()->text($release->body);
                }
                return $reference;
            }
            return null;
        ...
        public function getLatestTag()
        {
            ...
            $tag = $versionTags[0];
            return new \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Reference(array('name' => $tag->name, 'version' => \ltrim($tag->name, 'v'), 'downloadUrl' => $tag->zipball_url, 'apiResponse' => $tag));
        }
        /**
        ...
        public function getBranch($branchName)
        {
            ...
            $reference = new \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Vcs\Reference(array('name' => $branch->name, 'downloadUrl' => $this->buildArchiveDownloadUrl($branch->name), 'apiResponse' => $branch));
        ...
        protected function api($url, $queryParams = array())
        {
            ...
            $error = new \WPStack_Connect_Vendor\WP_Error('puc-github-http-error', \sprintf('GitHub API error. Base URL: "%s",  HTTP status code: %d.', $baseUrl, $code));
