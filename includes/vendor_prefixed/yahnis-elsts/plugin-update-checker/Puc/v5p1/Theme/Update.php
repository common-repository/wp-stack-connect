...
use WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Update as BaseUpdate;
if (!\class_exists(\WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Theme\Update::class, \false)) {
    class Update extends \WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\Update
    {
        public $details_url = '';
        ...
        protected function validateMetadata($apiResponse)
        {
            ...
            foreach ($required as $key) {
                if (!isset($apiResponse->{$key}) || empty($apiResponse->{$key})) {
                    return new \WPStack_Connect_Vendor\WP_Error('tuc-invalid-metadata', \sprintf('The theme metadata is missing the required "%s" key.', $key));
                }
            }
            return \true;
        }
