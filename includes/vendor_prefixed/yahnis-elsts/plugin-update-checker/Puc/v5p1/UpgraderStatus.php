...
if (!\class_exists(\WPStack_Connect_Vendor\YahnisElsts\PluginUpdateChecker\v5p1\UpgraderStatus::class, \false)) {
    /**
    ...
    class UpgraderStatus
    {
        ...
        private function getThingBeingUpgradedBy($upgrader)
        {
            ...
            $skin = $upgrader->skin;
            if (isset($skin->theme_info) && $skin->theme_info instanceof \WPStack_Connect_Vendor\WP_Theme) {
                $themeDirectoryName = $skin->theme_info->get_stylesheet();
            } elseif ($skin instanceof \WPStack_Connect_Vendor\Plugin_Upgrader_Skin) {
                if (isset($skin->plugin) && \is_string($skin->plugin) && $skin->plugin !== '') {
                    $pluginFile = $skin->plugin;
                }
            } elseif ($skin instanceof \WPStack_Connect_Vendor\Theme_Upgrader_Skin) {
        ...
        {
            if (!\function_exists('WPStack_Connect_Vendor\\get_plugins')) {
