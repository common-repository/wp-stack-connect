...
if (!\class_exists('WPStack_Connect_Vendor\\PucReadmeParser', \false)) {
    /**
    ...
    class PucReadmeParser
    {
        ...
        function parse_readme_contents($file_contents)
        {
            ...
            if (\preg_match('|Donate link:(.*)|i', $file_contents, $_donate_link)) {
                $donate_link = \WPStack_Connect_Vendor\esc_url($_donate_link[1]);
            } else {
                $donate_link = \NULL;
            }
        ...
        {
            // whitelisted chars
            if (\function_exists('WPStack_Connect_Vendor\\user_sanitize')) {
                // bbPress native
                return \WPStack_Connect_Vendor\user_sanitize($text, $strict);
            }
            if ($strict) {
        ...
        {
            // not fancy
            $text = \function_exists('WPStack_Connect_Vendor\\wp_strip_all_tags') ? \WPStack_Connect_Vendor\wp_strip_all_tags($text) : \strip_tags($text);
            $text = \WPStack_Connect_Vendor\esc_html($text);
            $text = \trim($text);
            return $text;
        }
        function filter_text($text, $markdown = \false)
        {
            ...
            if ($markdown) {
                // Parse markdown.
                if (!\class_exists('WPStack_Connect_Vendor\\Parsedown', \false)) {
                    /** @noinspection PhpIncludeInspection */
                    require_once \dirname(__FILE__) . '/Parsedown' . (\version_compare(\PHP_VERSION, '5.3.0', '>=') ? '' : 'Legacy') . '.php';
                }
                $instance = \WPStack_Connect_Vendor\Parsedown::instance();
            ...
            $text = \WPStack_Connect_Vendor\balanceTags($text);
            $text = \WPStack_Connect_Vendor\wp_kses($text, $allowed);
            $text = \trim($text);
            return $text;
        }
        ...
        {
            if (\function_exists('WPStack_Connect_Vendor\\encodeit')) {
                // bbPress native
                return \WPStack_Connect_Vendor\encodeit($matches);
            }
            $text = \trim($matches[2]);
        ...
        {
            if (\function_exists('WPStack_Connect_Vendor\\decodeit')) {
                // bbPress native
                return \WPStack_Connect_Vendor\decodeit($matches);
            }
            $text = $matches[2];
