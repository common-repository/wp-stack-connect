...
final class Env
{
    ...
    public static function createRuntime()
    {
        switch ($compileDir = self::getEnvVariable(self::COMPILE_DIR)) {
            case \false:
                return new \WPStack_Connect_Vendor\JmesPath\AstRuntime();
            case 'on':
                return new \WPStack_Connect_Vendor\JmesPath\CompilerRuntime();
            default:
                return new \WPStack_Connect_Vendor\JmesPath\CompilerRuntime($compileDir);
        }
    }
    /**
