 ...
 * @final
 */
class MessageFormatter implements \WPStack_Connect_Vendor\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\WPStack_Connect_Vendor\Psr\Http\Message\RequestInterface $request, ?\WPStack_Connect_Vendor\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \WPStack_Connect_Vendor\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
     ...
     */
    private function headers(\WPStack_Connect_Vendor\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
